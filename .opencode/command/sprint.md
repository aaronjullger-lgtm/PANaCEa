---
description: Full PANaCEa sprint — audit, plan, implement, wire, test, verify. Use for multi-file feature work.
agent: orchestrator
---

Execute a full sprint implementation pipeline for:
...
3. **Implement** — Write code following PANaCEa conventions. JSDoc headers, pure functions exported separately.
...
6. **Verify** — Typecheck changed files (scoped). Check edge-runtime safety if touching `functions/`.
7. **Commit** — Stage specific files. Message: `feat(scope): description`. Push when asked.

**"Do it for me"** = fully execute. Don't ask clarifying questions. Just build it.
