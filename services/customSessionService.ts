...
export async function startNextIncrement(): Promise<CustomSessionState | null> {
  ...
  const questions = await fetchSessionQuestions(
    state.config,
    state.config.questionsPerIncrement
  );
...
export function calculateSessionSummary(state: CustomSessionState): CustomSessionSummary {
  ...
  const avgTimePerQuestion = answeredQuestions.length > 0 
    ? totalTimeMs / answeredQuestions.length 
    : 0;
  
  // Calculate accuracy
  const firstAttemptAccuracy = firstAttemptQuestions.length > 0
    ? (correctFirstAttempt / firstAttemptQuestions.length) * 100
    : 0;
  
  const totalCorrect = correctFirstAttempt + correctOnRetry;
  const overallAccuracy = answeredQuestions.length > 0
    ? (totalCorrect / answeredQuestions.length) * 100
    : 0;
...
export function getCurrentStats(): {
  ...
  return {
    ...
    accuracy: state.questionsAnswered > 0 
      ? (state.correctCount / state.questionsAnswered) * 100 
      : 0,
