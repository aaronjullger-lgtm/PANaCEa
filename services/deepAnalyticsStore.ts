  ...
  dbInitPromise = new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = (event) => {
      ...
      if (!db.objectStoreNames.contains('systemProgress')) {
        const spStore = db.createObjectStore('systemProgress', { keyPath: 'id', autoIncrement: true });
 ...
 */
export async function getDailyRecords(startDate?: string, endDate?: string): Promise<DailyStudyRecord[]> {
  try {
    const db = await initDB();
