...
const ZPD = {
  floorAccuracy: 0.70, // Too easy if above 90%
  targetAccuracy: 0.80, // Ideal challenge
  ceilingAccuracy: 0.60, // Too hard if below 60%
};
...
export async function analyzeLearningEfficiency(): Promise<LearningEfficiencyMetrics> {
  ...
  const avgQuestionsToMastery = masteryQuestions.length > 0
  ...
  const optimalIntervals = reviewIntervals.filter(i => 
    i.actual >= i.optimal * 0.7 && i.actual <= i.optimal * 1.5
  ).length;
  const spacingAdherence = reviewIntervals.length > 0
    ? optimalIntervals / reviewIntervals.length
    : 0;
...
export async function analyzeMetacognition(): Promise<MetacognitiveProfile> {
  ...
  const highConfAcc = highConfidence.length > 0
    ? highConfidence.filter(a => a.isCorrect).length / highConfidence.length
    : 0.5;
  const lowConfAcc = lowConfidence.length > 0
  ...
  const overconfidenceRate = highConfidence.length > 0
    ? highConfidence.filter(a => !a.isCorrect).length / highConfidence.length
    : 0;
  ...
  const fatigueBreaks = attempts.filter((a, i) => 
    i > 0 && 
    a.fatigueLevel > 70 && 
    a.timestamp - attempts[i-1].timestamp > 300000 // 5+ min break
  ).length;
  const fatigueInstances = attempts.filter(a => a.fatigueLevel > 70).length;
  const selfMonitoringScore = fatigueInstances > 0
    ? Math.min(100, Math.round((fatigueBreaks / fatigueInstances) * 200))
    : 50;
...
export async function analyzeCognitiveLoad(): Promise<CognitiveLoadProfile> {
  ...
  for (const sessionAttempts of Object.values(sessionGroups)) {
    ...
    for (let i = 10; i < sessionAttempts.length - 5; i++) {
      ...
      const after = sessionAttempts.slice(i).filter(a => a.isCorrect).length / (sessionAttempts.length - i);
  ...
  const fatigueOnsetRate = fatiguePoints.length > 0
  ...
  const recoveryRate = breakRecoveries.length > 0
  ...
  const germaneLoadEfficiency = totalHighLoad > 0
    ? correctWithHighLoad / totalHighLoad
    : 0.5;
...
export async function analyzeCircadianPatterns(): Promise<CircadianProfile> {
  ...
  const hourlyPerformance = Object.entries(byHour).map(([hour, hourAttempts]) => ({
    hour: parseInt(hour),
    accuracy: hourAttempts.length > 10 
  ...
  const lowHours = hourlyPerformance
    .filter(h => h.accuracy < hourlyPerformance.reduce((sum, x) => sum + x.accuracy, 0) / hourlyPerformance.length * 0.85)
    .map(h => h.hour);
  ...
  const dailyPerformance = Object.entries(byDay).map(([day, dayAttempts]) => ({
    day: parseInt(day),
    accuracy: dayAttempts.length > 20
      ? dayAttempts.filter(a => a.isCorrect).length / dayAttempts.length
      : 0,
    count: dayAttempts.length,
  })).filter(d => d.count >= 20);
  
  const avgDayAcc = dailyPerformance.reduce((sum, d) => sum + d.accuracy, 0) / dailyPerformance.length;
...
export async function analyzeStrengthsWeaknesses(): Promise<StrengthWeaknessAnalysis> {
  ...
  for (const [system, sysAttempts] of Object.entries(systemGroups)) {
    ...
    // Calculate trend
    const recentAcc = recentGroups[system]?.length > 5
      ? recentGroups[system].filter(a => a.isCorrect).length / recentGroups[system].length
      : accuracy;
    const olderAcc = olderGroups[system]?.length > 5
    ...
    const daysToMastery = mastery >= 80 ? null : 
      Math.ceil((80 - mastery) / Math.max(0.5, trendDiff * 30 || 0.5));
...
export async function assessTestReadiness(): Promise<TestReadinessAssessment> {
  ...
  const recentAccuracy = attempts.slice(0, 200).filter(a => a.isCorrect).length / Math.min(200, attempts.length);
  const systemCoverage = new Set(attempts.map(a => a.system)).size / 13; // 13 PANCE systems
  const masteryAvg = (strengthsWeaknesses.strengths.length * 85 + 
    strengthsWeaknesses.weaknesses.length * 45) / 
  ...
  const passProbability = Math.round(
    Math.max(0, Math.min(100, (predictedScore - 400) / 2 + 50))
  );
  ...
  // Priority areas
  const priorityAreas = strengthsWeaknesses.weaknesses
    .slice(0, 3)
    .map(w => w.system);
  ...
  if (efficiency.spacingAdherence < 0.6) {
    recommendations.push({
      ...
      priority: 'high',
      evidence: 'Cepeda et al. (2006) found spaced practice produces 50% better long-term retention than massed practice',
  ...
  if (metacog.overconfidenceBias > 30) {
    recommendations.push({
      ...
      priority: 'high',
      evidence: 'Dunning-Kruger effect research shows overconfidence leads to underprepared test performance',
  ...
  if (cognitive.fatigueOnsetRate < 20) {
    recommendations.push({
      ...
      priority: 'medium',
      evidence: 'Cognitive load theory (Sweller, 1988) shows performance degrades with sustained mental effort',
  ...
  recommendations.push({
    ...
    priority: 'medium',
    evidence: 'Schmidt et al. (2007) demonstrated 10-20% performance variation based on circadian rhythms',
...
export async function generateUserFriendlyStats(): Promise<UserFriendlyStats> {
  ...
  const accuracyLifetime = totalQuestions > 0 ? Math.round((totalCorrect / totalQuestions) * 100) : 0;
  ...
  const last7DaysAccuracy = last7Days.length > 0
  ...
  const prevWeek = attempts.filter(a => 
    a.timestamp >= sevenDaysAgo - 7 * 24 * 60 * 60 * 1000 && 
    a.timestamp < sevenDaysAgo
  );
  
  const recentAcc = recentWeek.length > 0 
    ? recentWeek.filter(a => a.isCorrect).length / recentWeek.length
    : 0;
  const prevAcc = prevWeek.length > 0
    ? prevWeek.filter(a => a.isCorrect).length / prevWeek.length
    : recentAcc;
  ...
  const avgTimePerQuestion = attempts.length > 0
    ...
    : 0;
  const questionsPerHour = totalStudyHours > 0
    ? Math.round(totalQuestions / totalStudyHours)
    : 0;
  ...
  const helpfulChanges = withAnswerChanges.filter(a => 
    a.firstAnswer !== null && a.firstAnswer !== a.correctAnswer && a.finalAnswer === a.correctAnswer
  ).length;
  const harmfulChanges = withAnswerChanges.filter(a =>
    a.firstAnswer !== null && a.firstAnswer === a.correctAnswer && a.finalAnswer !== a.correctAnswer
  ).length;
  
  const firstInstinctCorrect = attempts.filter(a => 
    a.firstAnswer === a.correctAnswer
  ).length;
  const firstInstinctAccuracy = attempts.length > 0
    ? Math.round((firstInstinctCorrect / attempts.length) * 100)
    : 0;
  
  const answerChangeHelpfulness = withAnswerChanges.length > 0
  ...
  return {
    ...
    readinessLevel: readiness.readinessScore >= 80 ? 'ready' :
      readiness.readinessScore >= 65 ? 'almost_ready' :
      readiness.readinessScore >= 50 ? 'progressing' :
      readiness.readinessScore >= 30 ? 'building' : 'not_ready',
...
function groupBySession(attempts: QuestionAttemptRecord[]): Record<string, QuestionAttemptRecord[]> {
  const groups: Record<string, QuestionAttemptRecord[]> = {};
  for (const a of attempts) {
...
function groupByDifficulty(attempts: QuestionAttemptRecord[]): Record<string, QuestionAttemptRecord[]> {
  const groups: Record<string, QuestionAttemptRecord[]> = {};
  for (const a of attempts) {
...
function calculateReviewIntervals(attempts: QuestionAttemptRecord[]): Array<{ actual: number; optimal: number }> {
...
function calculateRetentionProjection(attempts: QuestionAttemptRecord[]): number {
  ...
  const spacingScore = intervals.length > 0
  ...
  const baseRetention = recent.length > 0
...
function generateSystemRecommendations(
  ...
  if (mastery < 50 && accuracy >= 0.6) {
    recommendations.push(`Increase practice frequency for ${system} - you understand but need consolidation`);
  }
...
function generateTopInsights(
  ...
  if (accuracyTrend === 'improving') {
    ...
    insights.push(`📉 Your accuracy dropped ${Math.abs(accuracyChange)}% - consider reviewing your weak areas`);
  }
  ...
  insights.push(`⏰ Your peak study time is ${circadian.peakLearningHour}:00 - ${(circadian.peakLearningHour + 2) % 24}:00`);
  insights.push(`😴 Take breaks every ${cognitive.fatigueOnsetRate} questions to maintain performance`);
