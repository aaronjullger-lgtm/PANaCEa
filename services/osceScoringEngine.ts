...
const CONDITION_CRITICAL_ACTIONS: Record<string, CriticalAction[]> = {
  ...
  'acute coronary syndrome': [
    { id: 'acs_ecg', category: 'diagnosis', description: 'Order ECG within 10 minutes', weight: 10, triggered: false, missedPenalty: 15 },
    { id: 'acs_troponin', category: 'diagnosis', description: 'Order serial troponins', weight: 9, triggered: false, missedPenalty: 10 },
    { id: 'acs_aspirin', category: 'procedure', description: 'Administer Aspirin 325mg', weight: 10, triggered: false, missedPenalty: 15 },
  ],
  'myocardial infarction': [
    { id: 'mi_ecg', category: 'diagnosis', description: 'Order ECG within 10 minutes', weight: 10, triggered: false, missedPenalty: 15 },
    { id: 'mi_troponin', category: 'diagnosis', description: 'Order serial troponins', weight: 9, triggered: false, missedPenalty: 10 },
    { id: 'mi_aspirin', category: 'procedure', description: 'Administer Aspirin 325mg', weight: 10, triggered: false, missedPenalty: 15 },
    { id: 'mi_cath_consult', category: 'procedure', description: 'Consider cardiology/cath lab consult', weight: 8, triggered: false, missedPenalty: 10 },
  ],
  
  // Stroke
  'stroke': [
    { id: 'stroke_ct', category: 'diagnosis', description: 'Order non-contrast head CT immediately', weight: 10, triggered: false, missedPenalty: 15 },
    { id: 'stroke_glucose', category: 'diagnosis', description: 'Check blood glucose', weight: 9, triggered: false, missedPenalty: 10 },
    { id: 'stroke_nihss', category: 'diagnosis', description: 'Calculate NIH Stroke Scale', weight: 8, triggered: false, missedPenalty: 8 },
    { id: 'stroke_time', category: 'communication', description: 'Document last known well time', weight: 10, triggered: false, missedPenalty: 12 },
  ],
  
  // Sepsis
  'sepsis': [
    { id: 'sepsis_lactate', category: 'diagnosis', description: 'Order serum lactate', weight: 9, triggered: false, missedPenalty: 10 },
    { id: 'sepsis_cultures', category: 'diagnosis', description: 'Obtain blood cultures before antibiotics', weight: 10, triggered: false, missedPenalty: 12 },
    { id: 'sepsis_abx', category: 'procedure', description: 'Start broad-spectrum antibiotics within 1 hour', weight: 10, triggered: false, missedPenalty: 15 },
    { id: 'sepsis_fluids', category: 'procedure', description: 'Initiate fluid resuscitation (30ml/kg)', weight: 9, triggered: false, missedPenalty: 12 },
  ],
  
  // Appendicitis
  'appendicitis': [
    { id: 'appy_rbq', category: 'diagnosis', description: 'Assess RLQ tenderness', weight: 8, triggered: false, missedPenalty: 6 },
    { id: 'appy_imaging', category: 'diagnosis', description: 'Order CT or ultrasound', weight: 8, triggered: false, missedPenalty: 8 },
    { id: 'appy_npo', category: 'procedure', description: 'Keep patient NPO', weight: 7, triggered: false, missedPenalty: 5 },
    { id: 'appy_surgical', category: 'procedure', description: 'Surgical consultation', weight: 9, triggered: false, missedPenalty: 10 },
  ],
  
  // Pneumonia
  'pneumonia': [
    { id: 'pna_cxr', category: 'diagnosis', description: 'Order chest X-ray', weight: 8, triggered: false, missedPenalty: 8 },
    { id: 'pna_o2', category: 'diagnosis', description: 'Assess oxygenation status', weight: 8, triggered: false, missedPenalty: 6 },
    { id: 'pna_curb65', category: 'diagnosis', description: 'Calculate severity score (CURB-65 or PSI)', weight: 7, triggered: false, missedPenalty: 5 },
    { id: 'pna_abx', category: 'procedure', description: 'Start appropriate antibiotics', weight: 9, triggered: false, missedPenalty: 10 },
  ],
  
  // Pulmonary Embolism
  'pulmonary embolism': [
    { id: 'pe_wells', category: 'diagnosis', description: 'Calculate Wells score for PE', weight: 8, triggered: false, missedPenalty: 6 },
    { id: 'pe_ddimer', category: 'diagnosis', description: 'Order D-dimer if appropriate', weight: 7, triggered: false, missedPenalty: 5 },
    { id: 'pe_ctpa', category: 'diagnosis', description: 'Order CT pulmonary angiogram', weight: 9, triggered: false, missedPenalty: 12 },
    { id: 'pe_anticoag', category: 'procedure', description: 'Initiate anticoagulation', weight: 9, triggered: false, missedPenalty: 12 },
  ],
};
...
export class OSCEScoringEngine {
  ...
  trackOrder(order: PlacedOrder): ScoringEvent | null {
    ...
    // Chest X-ray
    if (orderName.includes('chest') && (orderName.includes('x-ray') || orderName.includes('xray') || orderName.includes('cxr'))) {
    ...
    // CT PE
    if (orderName.includes('ct') && (orderName.includes('pe') || orderName.includes('pulmonary') || orderName.includes('angio'))) {
  ...
  trackTreatment(treatment: string): ScoringEvent | null {
    ...
    if (/\bantibiotic|ceftriaxone|azithromycin|levofloxacin|vancomycin|piperacillin|meropenem\b/.test(lowerTx)) {
    ...
    if (/\bheparin|lovenox|enoxaparin|anticoag|eliquis|xarelto|apixaban|rivaroxaban\b/.test(lowerTx)) {
   ...
   */
  private triggerCriticalAction(actionId: string, phase: TimelineEntry['phase']): ScoringEvent | null {
  ...
  calculateCompetencyScores(): CompetencyScore {
    ...
    scores.history = Math.min(100, 50 + (this.timeline.filter(t => t.phase === 'history').length * 5));
    scores.physicalExam = Math.min(100, 40 + (this.timeline.filter(t => t.phase === 'physical').length * 8));
    ...
    scores.efficiency = elapsedMinutes < 10 ? 100 : elapsedMinutes < 15 ? 85 : elapsedMinutes < 20 ? 70 : 55;
  ...
  generateExpertComparisons(): ExpertComparison[] {
    ...
    this.criticalActions.forEach(action => {
      const expertTiming = action.category === 'safety' ? 60 : // 1 minute
                          action.category === 'diagnosis' ? 300 : // 5 minutes
                          action.category === 'communication' ? 120 : // 2 minutes
  ...
  generateReport(finalDiagnosis?: string, differentials?: string[]): OSCEScoreReport {
    ...
    const diagnosisCorrect = finalDiagnosis && 
