...
export function getLearningVelocity(): LearningVelocity {
  ...
  const retentionEfficiency = totalIntroduced > 0 ? (totalRetained / totalIntroduced) * 100 : 85;

  // Trend analysis
  let trend: 'accelerating' | 'stable' | 'decelerating' = 'stable';
  if (older.length >= 3) {
    const recentRate = average(
      recent.map((d) => d.questionsCorrect / (d.sessionDurationMs / 3600000))
    );
    const olderRate = average(
      older.map((d) => d.questionsCorrect / (d.sessionDurationMs / 3600000))
    );
...
export function generateComprehensiveAnalytics(
  ...
  const avgMastery =
    systemMastery.length > 0 ? average(systemMastery.map((s) => s.masteryLevel)) : 50;
  const predictedPANCEScore = Math.round(
    200 + (overallAccuracy / 100) * 400 + (avgMastery / 100) * 200
  );

  // Pass probability based on predicted score (passing ~450)
  const predictedPassProbability = Math.min(
    99,
    Math.max(1, 50 + (predictedPANCEScore - 450) * 0.5)
  );
...
function generateInsights(
  ...
  if (cognitive.flowState > 75) {
    insights.push({
      ...
      description:
        "You're experiencing optimal focus. This is a great time to tackle challenging content.",
      actionable: false,
    });
  }
  ...
  if (velocity.trend === 'accelerating') {
    ...
    insights.push({
      ...
      description:
        'Your learning velocity has decreased. Consider reviewing your study approach or taking a longer break.',
      actionable: true,
