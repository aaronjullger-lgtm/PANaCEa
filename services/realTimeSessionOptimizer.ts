...
export interface SessionOptimization {
  ...
  recommendedAction: 'continue' | 'take_break' | 'switch_topic' | 'lower_difficulty' | 'end_session';
  reason: string;
...
export function analyzeCognitiveState(
  ...
  const variance = responseTimes.reduce((sum, t) => sum + Math.pow(t - avgTime, 2), 0) / (responseTimes.length || 1);
  ...
  const isInFlow = recentAccuracy > 0.7 && avgTime > 15000 && avgTime < 45000 && variance < 100000000;
  const flowState = isInFlow ? Math.min(100, 70 + (recentAccuracy - 0.7) * 100) : Math.max(0, recentAccuracy * 50);
...
export function calculateOptimalSessionLength(
  ...
  const avgOptimalDuration = topSessions.reduce((sum, s) => sum + s.duration, 0) / topSessions.length;

  // Adjust for time of day
  let timeAdjustment = 1;
  if (timeOfDay >= 6 && timeOfDay <= 10) timeAdjustment = 1.1; // Morning boost
  else if (timeOfDay >= 14 && timeOfDay <= 16) timeAdjustment = 0.9; // Post-lunch dip
