...
export class PANCEScorePredictorService {
  ...
  static predictScore(
    ...
    const projectedDate = passLikelihood < 80 
      ? this.projectPassDate(performanceData, passLikelihood) 
      : null;
   ...
   */
  private static calculateDifficultyAdjustedPerformance(
    ...
    return (
      bandAccuracy.easy * 0.2 +
      bandAccuracy.medium * 0.5 +
      bandAccuracy.hard * 0.3
    );
  }
   ...
   */
  private static determineReadinessLevel(
    passLikelihood: number
  ): PredictedScore["readinessLevel"] {
  ...
  private static generateRecommendations(
    ...
    if (lowCoverageSystems.length > 0) {
      recommendations.push(
        `Increase coverage in: ${lowCoverageSystems.slice(0, 3).join(", ")}`
      );
    }
   ...
   */
  private static getInsufficientDataResponse(
    questionsAnswered: number
  ): PredictedScore {
    return {
      scaledScore: 0,
