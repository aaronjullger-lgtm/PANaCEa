  ...
  event.waitUntil(
    caches.keys()
      .then((cacheNames) => {
        return Promise.all(
          cacheNames
            .filter((name) => name.startsWith("panacea-") && name !== STATIC_CACHE && name !== DYNAMIC_CACHE && name !== QUESTION_CACHE)
            .map((name) => {
...
function isStaticAsset(pathname) {
  const staticExtensions = [".js", ".css", ".png", ".jpg", ".jpeg", ".svg", ".gif", ".woff", ".woff2", ".ico"];
