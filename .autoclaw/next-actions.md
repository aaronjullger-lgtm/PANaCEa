...
1. [ ] Run full repo discovery pass — file counts, test health, build status
2. [ ] Identify any failing tests, broken routes, disconnected UI
3. [ ] Verify build passes clean: `npm run build`
4. [ ] Verify typecheck: `NODE_OPTIONS="--max-old-space-size=4096" npx tsc --noEmit`
...
3. [ ] QuizView refactor — parked on `wip/quizview-refactor-parked` (192 TS errors)
4. [ ] Drill routing consolidation — DrillShell vs. useDrillFSRS split
