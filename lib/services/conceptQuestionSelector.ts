...
async function fetchDueReviews(
  ...
  for (const progress of dueProgress) {
    ...
    selected.push(normalizeQuestion(picked, 'due_review'));
  }
...
async function fetchScopedNew(
  ...
  return rankQuestionRows(
    ...
    'new_card',
    options.snapshot,
    options.blueprintWeights,
    attemptedSet
  )
    .slice(0, count)
    .map((entry) => normalizeQuestion(entry.row, 'new_card'));
}
...
async function fetchAdaptiveNew(
  ...
  for (const system of systems) {
    ...
    for (const entry of ranked.slice(0, targetCount)) {
      const question = normalizeQuestion(entry.row, 'new_card');
      selected.push(question);
      sessionExcluded.add(question.id);
    }
  }
...
function rankQuestionRows(
  ...
  return rows
    .filter((row) => !excludeQuestionIds.has(row.id))
    .map((row) => {
      const descriptor: AdaptiveCandidateDescriptor = {
        questionId: row.id,
        system: row.system,
        conditionId: row.conditionId,
        source,
        blueprintWeight: blueprintWeights[row.system ?? ''] ?? 0,
        overdueDays,
      };
      const score = scoreAdaptiveCandidate(descriptor, snapshot);
      const preferredBonus = preferredQuestionId && row.id === preferredQuestionId ? 4 : 0;
      return {
        row,
        score: score.total + preferredBonus,
      };
    })
    .sort((left, right) => {
      ...
      const leftSystem = left.row.system ?? 'Unknown';
      const rightSystem = right.row.system ?? 'Unknown';
      if (leftSystem !== rightSystem) {
        return leftSystem.localeCompare(rightSystem);
      }
      const leftCondition = left.row.conditionId ?? '';
      const rightCondition = right.row.conditionId ?? '';
...
function buildMetadata(
  ...
  mode: SessionMode,
  blueprintStage: LearnerStage
): SessionGenerationResult['metadata'] {
  ...
  return {
    dueReviewCount: dueReviewCount + resurfacedCount,
    newCardCount,
    systemDistribution,
    estimatedMinutes: Math.round((questions.length * AVG_SECONDS_PER_QUESTION) / 60),
    mode,
    blueprintStage,
    adaptiveVersion: 'v1',
    selectionMix: {
