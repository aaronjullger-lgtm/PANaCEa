...
const PATHOGNOMONIC_TERMS: string[] = [
  ...
  "Owl's eye inclusion", "owl's eye", "owl eye",
  "Reed-Sternberg cells", "auer rods", "Heinz bodies",
  "apple core lesion", "string sign", "double bubble sign",
  "café-au-lait spots", "ash leaf spots", "port-wine stain",
  "strawberry tongue", "target lesion", "herald patch",
  "chocolate cyst", "codman triangle", "sunburst pattern",
  "bamboo spine", "pencil-in-cup", "soap bubble appearance",
  "ground glass", "tree-in-bud", "crazy paving",
  
  // Cardiovascular
  'ST elevation', 'troponin', 'STEMI', 'NSTEMI',
  'pulsus paradoxus', 'Kussmaul sign', "Beck's triad", 'JVD',
  'water-hammer pulse', 'opening snap', 'Austin Flint murmur',
  ...
  'currant jelly stool', 'rice-water stool', 'acholic stool',
  
  // Neuro
  'Babinski sign', 'clonus', 'pronator drift', 'photophobia',
  'Kernig sign', 'Brudzinski sign', 'nuchal rigidity', 'papilledema',
  'Lhermitte sign', 'Romberg sign', 'Chvostek sign', 'Trousseau sign',
  
  // Derm
  'maculopapular', 'vesicular', 'erythematous',
  'Nikolsky sign', 'Auspitz sign', 'Koebner phenomenon',
...
const MNEMONIC_DATABASE: Record<string, string> = {
  ...
  'pulmonary_embolism': 'PE Risk: THROMBOSIS - T-Trauma, H-Hypercoagulable, R-Replaced hip/knee, O-Oral contraceptives, M-Malignancy, B-Bedrest, O-Obesity, S-Surgery, I-Immobility, S-Stroke',
  ...
  'sickle_cell': 'Sickle Cell Crises: HAVES - H-Hand-foot syndrome, A-Aplastic, V-Vaso-occlusive, E-Enlarged spleen (sequestration), S-Stroke',
...
export function compressToBullets(text: string): string[] {
  ...
  const scoredSentences = sentences.map(sentence => {
    ...
    if (/\b(diagnos|confirm|rule out|first-line|gold standard|most common|classic|typical|pathognomonic)\b/i.test(sentence)) {
      score += 3;
    }
    
    // Boost sentences with treatment information
    if (/\b(treat|manag|therap|medicat|prescribe|administer|first[\s-]?line|drug of choice)\b/i.test(sentence)) {
      score += 3;
    }
    
    // Boost sentences with differentiating information
    if (/\b(unlike|versus|vs\.|contrast|distinguish|differentiate|whereas|compared to|differs from)\b/i.test(sentence)) {
      score += 3;
    }
