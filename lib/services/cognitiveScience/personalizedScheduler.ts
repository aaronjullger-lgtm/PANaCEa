...
export function determineChronotype(
  ...
  if (sleepMidpoint < 2.5 && mostAlertTimeOfDay === 'morning') {
    ...
    sleepMidpoint > 5.5 &&
    (mostAlertTimeOfDay === 'evening' || mostAlertTimeOfDay === 'night')
  ) {
    type = 'strong-evening';
    peakAlertHour = 21;
...
export function generateDailySchedule(
  ...
  const sortedWindows = [...chronotype.optimalStudyWindows].sort((a, b) => {
  ...
  const peakMinutes = blocks
    .filter((b) => b.isOptimalTime)
  ...
  if (
    newTopics.length > 0 &&
    !blocks.some((b) => b.contentType === 'new-learning' && b.isOptimalTime)
  ) {
    recommendations.push(
...
export function calculateExamOptimizedSpacing(
  ...
  if (urgency === 'critical') {
    ...
    daysToNextReview = Math.min(
      currentStability,
      Math.floor(daysUntilExam / (reviewsBeforeExam + 1))
    );
  }
...
export function generatePersonalizedTips(
  ...
  if (chronotype.type.includes('morning')) {
    ...
    tips.push(
      '⚖️ Your intermediate chronotype gives you flexibility. Use mid-morning and late afternoon.'
    );
  }
