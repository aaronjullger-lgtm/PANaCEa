...
function calculateSessionContextSignal(
  ...
  // Fatigue adjustment
  const fatigueMultiplier = questionNumber > fatigueThreshold 
...
function calculateHesitationSignal(
  ...
  const avgClickInterval = timeBetweenClicks.length > 0
...
export function inferRating(
  ...
  weightedSignals.push({
    ...
    contribution: stabilityResult.signal * 0.20,
  });
...
export function buildPerformanceProfile(
  ...
  const correctRate = historicalResponses.filter(r => r.isCorrect).length / historicalResponses.length;
