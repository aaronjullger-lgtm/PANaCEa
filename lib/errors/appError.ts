...
export class AppErrorFactory {
  ...
  private static getUserFriendlyMessage(
    ...
    const messageMap: Record<ErrorCode, string> = {
      ...
      [ErrorCode.AI_SERVICE_CONTENT_FILTER]:
        'Content could not be generated due to safety filters.',
   ...
   */
  private static getRecoveryActions(category: ErrorCategory, code: ErrorCode): RecoveryAction[] {
...
export function shouldReportError(error: AppError): boolean {
  const nonReportableCategories = [ErrorCategory.VALIDATION, ErrorCategory.RATE_LIMIT];
  ...
  return !(
    nonReportableCategories.includes(error.category) || nonReportableCodes.includes(error.code)
  );
}
