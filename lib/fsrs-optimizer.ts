...
import { defaultParameters, type FSRSParameters, type ReviewSnapshot, Rating, FSRSState } from './fsrs';
...
export function computeBrierScore(
  ...
  for (const review of reviews) {
    ...
    const predicted = computeRetrievability(
      review.elapsedDays,
      review.stability,
      w
    );
...
export function optimizeFSRSParameters(
  ...
  // Compute improvement
  const improvement = defaultBrierScore > 0
    ? ((defaultBrierScore - result.value) / defaultBrierScore) * 100
    : 0;
...
export function optimizeWithSystemModifiers(
  ...
  for (const [system, systemReviews] of Object.entries(reviewsBySystem)) {
    ...
    const avgStability = systemReviews.reduce((sum, r) => sum + r.stability, 0) / systemReviews.length;
    const avgDifficulty = systemReviews.reduce((sum, r) => sum + r.difficulty, 0) / systemReviews.length;
    ...
    const globalAccuracy = baseParams.sampleSize > 0
