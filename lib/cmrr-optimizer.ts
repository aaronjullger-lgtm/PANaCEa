...
const DEFAULT_W = [
  0.4072, 1.1829, 3.1262, 15.4722,  // Initial stability (S0)
  7.2102, 0.5316, 1.0651,          // Difficulty-stability relationship
  0.0652, 1.5351, 0.1367, 1.0450,  // Memory strength decay
  1.9395, 0.1091, 0.2995, 2.2584,  // Forgetting curve parameters
  0.2898, 2.8190, 0.5170, 0.6050, // Additional v5 parameters
];
...
export function calculateOptimalRetention(input: CMRRInput): CMRROutput {
  ...
  const retentionSamples = [0.80, 0.82, 0.85, 0.87, 0.90, 0.92, 0.95, 0.97];
  ...
  if (targetExamDate) {
    const daysUntilExam = Math.max(1, (targetExamDate.getTime() - Date.now()) / (1000 * 60 * 60 * 24));
  ...
  const confidenceLevel = 
    reviewHistory.length >= 200 ? 'high' :
    reviewHistory.length >= 100 ? 'medium' : 'low';
  
  // Generate recommendation
  const recommendation = generateRecommendation(optimalR, currentParams?.requestR || DEFAULT_RETENTION, avgStudyTimeMinutes);
  
  // Calculate slider position (0-100)
  const sliderPosition = Math.round(((optimalR - MIN_RETENTION) / (MAX_RETENTION - MIN_RETENTION)) * 100);
...
export function retentionToLabel(r: number): string {
  ...
  if (r >= 0.90) return 'High Retention';
  if (r >= 0.85) return 'Balanced';
  if (r >= 0.80) return 'Efficiency Focus';
