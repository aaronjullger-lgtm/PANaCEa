/**
 * generate-enhanced orchestrator strategy.
 *
 * Wraps the POST /api/questions/generate-enhanced flow (PANCE question
 * generation with Chain of Verification: gateway draft → CoVe verify →
 * staging lake write → conditional live promotion) as a registered
 * orchestrator agent with typed input/output state and standard logging.
 * The endpoint delegates to `runGenerateEnhancedFlow` — one source of truth.
 *
 * Ported from functions/api/questions/generate-enhanced.ts. Gateway mapping
 * preserved: main generation task='generation', tier='powerful'
 * (gemini-2.5-pro, temp 0.75); CoVe verification wrapper task='grading',
 * tier='balanced' (lower default temperature for deterministic fact-checks).
 *
 * @module lib/agents/strategies/generateEnhancedStrategy
 */
 ...
 */
export function parseConditionContext(contextString: string): ParsedConditionContent {
...
export const GenerateEnhancedInputSchema = z.object({
...
export type GenerateEnhancedInput = z.infer<typeof GenerateEnhancedInputSchema>;
...
// without a full re-generation — the flow aborts rather than polluting the pipeline.
const MIN_CONFIDENCE_FOR_STAGING = 0.35;

/**
 * Build the Kaplan-style generation prompt. Ported verbatim from the endpoint.
 * `previousIssues` injects CoVe verification failures from prior attempts so
 * the model regenerates with extra attention to medical accuracy.
 */
export function buildGenerationPrompt(params: {
  ...
  const { conditionContext, taskInstruction, difficultyInstruction } = params;
  let issueWarning = '';
  if (params.previousIssues && params.previousIssues.length > 0) {
    issueWarning = `\n## IMPORTANT: Previous Generation Issues (MUST FIX)
The previous attempt had these verification failures:
${params.previousIssues.map((issue) => `- ${issue}`).join('\n')}
...
export interface GenerateEnhancedVerificationReport {
  verified: boolean;
  confidence: number;
  attempts: number;
...
export interface GenerateEnhancedHeldForReviewData {
  success: false;
  submissionReady: false;
  requiresApproval: true;
  stagingQuestionId: string;
  message: string;
  verification: GenerateEnhancedVerificationReport;
}

export interface GenerateEnhancedVerifiedData {
  success: true;
  question: {
    id: string;
    vignette: string;
...
export type GenerateEnhancedFlowOutput =
  | { status: 202; data: GenerateEnhancedHeldForReviewData }
  | { data: GenerateEnhancedVerifiedData };

export const GenerateEnhancedFlowOutputSchema = z.object({
  status: z.number().int().optional(),
  data: z.unknown(),
});
export type GenerateEnhancedFlowOutputState = z.infer<typeof GenerateEnhancedFlowOutputSchema>;

export interface GenerateEnhancedFlowDeps {
  env: { GEMINI_API_KEY?: string; DATABASE_URL?: string };
...
/**
 * Run the generate-enhanced flow: gateway draft → CoVe verification loop →
 * staging lake write → conditional live promotion (only when verification
 * passed). Returns the same response envelope the endpoint returned
 * ({status: 202, data} when held for review, {data} when promoted).
 * Throws on fatal failures — callers decide how to surface (the endpoint's
 * error handling / the agent's invoke wrapper).
 */
export async function runGenerateEnhancedFlow(
  input: GenerateEnhancedInput,
  deps: GenerateEnhancedFlowDeps,
): Promise<GenerateEnhancedFlowOutput> {
  const { context: conditionContext, conditionId, conditionName, system, task, difficulty } = input;
  const logger = deps.logger;

  if (!deps.env.DATABASE_URL) {
    throw new Error('DATABASE_URL is required to run the generate-enhanced flow');
  }
  const prisma = createEdgePrismaClient(deps.env.DATABASE_URL);

  try {
    // AI Gateway: build GatewayContext once, reuse for all calls in this request.
    const gwCtx = toGatewayContext({ env: deps.env, auth: deps.auth, waitUntil: deps.waitUntil });
    ...
    while (attempt < MAX_COVE_RETRIES) {
      ...
      try {
        ...
        questionData = JSON.parse(cleanedResponse) as QuestionData;
      } catch (parseError) {
      ...
      try {
        if (USE_FULL_COVE) {
          ...
          logger.info(`[CoVe] Running full verification pipeline`, { userId: deps.auth?.userId });
          ...
          logger.info(`[CoVe] Running quick verification`, { userId: deps.auth?.userId });
    ...
    try {
      await prisma.stagingQuestion.create({
        data: {
          ...
          explanation:
            typeof questionData.rationale === 'string'
              ? questionData.rationale
...
export const generateEnhancedAgent: AgentDefinition<
  GenerateEnhancedInput,
  GenerateEnhancedFlowOutput
> = {
  name: 'generate-enhanced',
  description:
    'Generate a PANCE question with Chain of Verification: gateway draft, CoVe verify, staging lake write, conditional live promotion.',
  tier: 'ops',
  inputSchema: GenerateEnhancedInputSchema,
  outputSchema: GenerateEnhancedFlowOutputSchema,
  async invoke(input, ctx) {
    const start = Date.now();
    const parsed = GenerateEnhancedInputSchema.safeParse(input);
    if (!parsed.success) {
      return {
        ...
        error: {
          status: 'schema_invalid',
          message: `Invalid generate-enhanced input: ${parsed.error.message}`,
          cause: 'input_schema',
        },
        agent: 'generate-enhanced',
        durationMs: 0,
      };
    }
    ...
    try {
      const output = await runGenerateEnhancedFlow(parsed.data, {
        env: { ...ctx.env, DATABASE_URL: (ctx.env as { DATABASE_URL?: string }).DATABASE_URL },
      ...
      ctx.log?.('error', '[generate-enhanced] flow failed', {
        error: message,
        userId: ctx.userId,
      });
      return {
        ...
        output: null,
        error: { status: 'internal_error', message, cause: 'generate_enhanced_flow' },
        agent: 'generate-enhanced',
        durationMs: Date.now() - start,
      };
    }
...
/**
 * Register the generate-enhanced strategy in the runtime registry.
 * Idempotent for the same definition; throws on conflicting registrations.
 */
export function registerGenerateEnhancedStrategy(): void {
  registerAgent(generateEnhancedAgent);
}

// Self-register on import so `lib/agents/registry` picks the strategy up.
registerGenerateEnhancedStrategy();
