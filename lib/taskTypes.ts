...
export function getTaskTypeFromContent(content: string, title?: string): TaskType {
    ...
    if (lowerContent.includes('treatment') || lowerContent.includes('management') || lowerContent.includes('therapy')) {
        return TASK_TYPES.TREATMENT;
    }
    if (lowerContent.includes('diagnos') || lowerContent.includes('presentation') || lowerContent.includes('symptom')) {
        return TASK_TYPES.DIAGNOSIS;
    }
    if (lowerContent.includes('mechanism') || lowerContent.includes('pathophys')) {
        return TASK_TYPES.MECHANISM;
    }
    if (lowerContent.includes('workup') || lowerContent.includes('test') || lowerContent.includes('lab')) {
