 ...
 */
function classifyRhythm(analysis: Omit<TypingAnalysis, 'rhythm' | 'confidence'>): { rhythm: TypingRhythm; confidence: number } {
  ...
  const hesitantPauses = pauses.distribution.cognitive + pauses.distribution.reflective + pauses.distribution.stuck;
 ...
 */
function calculateCognitiveLoad(pauses: PauseAnalysis, deletionRatio: number, ikiCV: number): number {
  ...
  const cognitivePauses = pauses.distribution.cognitive + pauses.distribution.reflective + pauses.distribution.stuck;
  ...
  return (
    ...
    deletionWeight * deletionScore +
    variabilityWeight * variabilityScore
  );
}
...
function calculateFluencyScore(
  ...
  const ikiScore = avgIKI > 100 && avgIKI < 300 
    ? 1 - Math.abs(avgIKI - 175) / 175
    : 0.5;
...
export function analyzeTypingSession(events: KeystrokeEvent[]): TypingAnalysis {
  ...
  // IKI statistics
  const ikiStats = calculateIKIStats(productions.length > 1 ? productions.map((p, i) => events.find(e => e === p)!) : events);

  // Extract pauses
  const pauseEvents = extractPauses(events);
  const pauseDistribution: Record<PauseType, number> = {
    physical: 0, lexical: 0, cognitive: 0, reflective: 0, stuck: 0
  };
  ...
  const pauses: PauseAnalysis = {
    ...
    avgPauseDuration: pauseEvents.length > 0 
      ? pauseEvents.reduce((s, p) => s + p.durationMs, 0) / pauseEvents.length 
      : 0,
    maxPause: pauseEvents.length > 0 
      ? Math.max(...pauseEvents.map(p => p.durationMs)) 
      : 0,
  ...
  const bursts: BurstAnalysis = {
    ...
    avgBurstLength: burstSegments.length > 0 
      ? totalBurstChars / burstSegments.length 
      : 0,
    burstCoverage: productions.length > 0 
      ? totalBurstChars / productions.length 
      : 0,
    avgBurstIKI: burstSegments.length > 0 
