...
- **Verified working:** `npm run build`, Prisma client generation, `npm run test:critical` (143 tests), and the full `npm test` suite (527 files, ~9849 tests, ~140s). `npm run lint` currently exits non-zero from 3 pre-existing `no-empty` errors (plus ~250 warnings, under the 2000-warning gate) — unrelated to environment setup.
