...
Standard commands live in `README.md`, the local dev runbook markdown at the repo root, `CLAUDE.md`, and `package.json` scripts. The startup update script runs `npm install` (which triggers `postinstall` → `prisma generate`). Node 22 is already present. Notes below are the non-obvious cloud caveats:

- **Secrets are injected as environment variables** (no `.env` needed for values): `DATABASE_URL` (a `prisma://` Accelerate URL, used by the edge `functions/api/`), `DIRECT_DATABASE_URL` (a `postgres://` URL for CLI/scripts/migrations), `CLERK_SECRET_KEY`, `GEMINI_API_KEY`, `SUPABASE_*`, and the `VITE_*` public keys. Vite (`npm run dev`) reads `VITE_*` straight from the process env, so the frontend needs no `.env`.
- **Frontend dev works:** `npm run dev` serves the SPA on port 3000 (renders the PANCE Command Center landing page). Its Vite proxy forwards `/api` and `/geminiProxy` to `http://localhost:3001` (the Express server), so data-backed screens only work if a backend is reachable there.
- **Express dev path is currently broken on `main`:** `server.ts` imports `./routes`, but that directory does not exist (moved to `_trash/old-routes/`), so `npm run dev:server` / `npm run dev:all` crash with `ERR_MODULE_NOT_FOUND`. Separately, those scripts launch with `node --env-file=.env`, which hard-fails if no `.env` file exists — `touch .env` first (an empty file is enough since real values come from the injected env). Do not add `routes/` back as a setup step; treat it as a code-level recovery task.
- **Wrangler production-parity path is also broken on `main`:** `npm run dev:wrangler` (build → `wrangler pages dev dist` on port 8788) fails to bundle Functions because `functions/api/_shared/semantic-cache.ts` imports `@/lib/services/tokenMatchCache`, which does not exist. If/when that file is restored, pass Function secrets to wrangler via a gitignored `.dev.vars` file (dotenv format, e.g. `DATABASE_URL=...`, `CLERK_SECRET_KEY=...`, `GEMINI_API_KEY=...`, `SUPABASE_SERVICE_ROLE_KEY=...`) built from the injected env — `wrangler pages dev` does not inherit `process.env` into `context.env`.
- **DB scripts:** connect with `DIRECT_DATABASE_URL`, strip the query string, and pass `ssl: { rejectUnauthorized: false }`. The `pg` driver treats `sslmode=require` as `verify-full` and otherwise fails with "self-signed certificate in certificate chain" against Supabase.
- **Verified working:** `npm run build`, `npm run test:critical` (143 tests), and Prisma client generation. `npm run lint` currently exits non-zero from 3 pre-existing `no-empty` errors (plus ~250 warnings, under the 2000-warning gate) — unrelated to environment setup. The full `npm test` suite has known-failing files (React 19 `components/admin`, Goals, offline sync) per `CLAUDE.md`.
