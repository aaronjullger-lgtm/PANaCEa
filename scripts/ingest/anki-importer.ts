    ...
    try {
      ...
      for (const note of notes) {
        try {
          ...
          /* Duplicate check adjusted for schema validity */
          /* Content hash removed as it does not exist on Question model */
          const existing = await prisma.question.findFirst({
            where: { question: parsed.front },
          });
          ...
          // Import to database
          await this.saveQuestion(parsed);
          stats.imported++;
