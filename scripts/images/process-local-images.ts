...
const FILENAME_TO_CONDITION_MAP: Record<string, string> = {
  ...
  'Erythema_Migrans': 'DERM__infectious_bacterial_systemic_with_cutaneous_manifestations__erythema_migrans',
...
const CLINICAL_FILENAME_MAP: Record<string, string> = {
  ...
  'ErythemaMigrans': 'DERM__infectious_bacterial_systemic_with_cutaneous_manifestations__erythema_migrans',
  ...
  'AnteriorShoulderDislocation_Squared-offshoulderX-ray': 'MSK__traumafracture__anterior_shoulder_dislocation',
  ...
  'SupracondylarFracture_Posteriorfatpad-Anteriorhumeralline': 'MSK__traumafracture__supracondylar_humerus_fracture',
  ...
  'SubconjunctivalHemorrhage_Bloodpatchonsclera': 'HEENT__eye__conjunctiva__subconjunctival_hemorrhage',
  ...
  'DendriticKeratitis_Branchingulcer': 'HEENT__eye__cornea__anterior_segment__herpes_simplex_keratitis',
  ...
  'WaterBottleHeart_EnlargedcardiacsilhouetteEffusion': 'CV__pericardial_disease__pericardial_effusion',
  ...
  'InfectiousTenosynovitis_KanavelSignsSausagedigit': 'DERM__infectious_bacterial__infectious_tenosynovitis',
  'Dermatitis_herpetiformis': 'DERM__bullous__dermatitis_herpetiformis',
  ...
  'CongenitalHypothyroidism_Macroglossia-Umbilicalhernia': 'ENDO__thyroid__congenital_hypothyroidism',
  ...
  'Second_Degree_AV_Block_Mobitz_I__Wenckebach_': 'CV__conduction_disorders__second_degree_av_block_mobitz_i',
...
function extractConditionName(filename: string): string {
  ...
  const lectureIndicators = [
    /^EM\s/i, /^CM\s/i, /^MSK\s/i, /^DERM\s/i, /^GI\s/i, /^CV\s/i, /^PULM\s/i,
    /^NEURO/i, /^HEENT/i, /^ID\s/i, /^ENDO/i, /^HEME/i, /^RENAL/i, /^GU\s/i,
    /Hemorrhage$/i, /Disorders$/i, /Disease$/i, /Diseases$/i, /Conditions$/i,
    /Infections$/i, /Neoplasms$/i, /Syndrome$/i, /Treatment$/i, /Management$/i,
    /^Intracranial/i, /^Behavioral/i, /^Emotional/i, /^Breast/i, /^Menstrual/i,
    /^Menopause/i, /^Pregnancy/i, /^Kidney/i, /^Surgical/i, /^Asthma/i,
    /^OBGYN/i, /^SURGERY/i, /^Arthritis$/i, /^Rheumatology$/i, /^Carditis$/i,
    /^Spirochetal$/i, /^Immunizations$/i, /^Sepsis$/i, /^Pleural$/i, /^COPD$/i,
    /^Hip$/i, /^Knee$/i, /^Lower\s/i, /^Upper\s/i, /^Hand\s/i, /^Wrist$/i,
    /^Gallbladder$/i, /^Esophagus$/i, /^Anorectal$/i, /^Movement$/i,
    /^Benign\s/i, /^Acneiform$/i, /^Biologics$/i, /^Gynecologic$/i,
    /^The\s/i, /^Red\s/i, /^Painful$/i, /Eye$/i, /^Ear$/i, /^Prostate$/i,
    /^Oxygen$/i, /^Therapy$/i, /^ARDS$/i, /^Measles$/i, /^ABG$/i,
    /^Septic$/i, /^Seizure$/i, /^CKD$/i, /^ENVIRONMENTAL$/i, /^EXPOSURES$/i,
    /^Therapeutics$/i, /^Complicated$/i, /^Infertility$/i, /^Mumps$/i,
    /^Parathyroid$/i, /^SHOCK$/i, /^Adrenal$/i, /^Corneal$/i, /^Ulcer$/i,
    /^Vulva$/i, /^Vaginal$/i, /^Myco$/i, /^Fungal$/i,
  ];
...
function findConditionId(conditionName: string, conditions: Map<string, string>): string | null {
  ...
  const normalizedSearch = conditionName.toLowerCase().replace(/_/g, ' ').replace(/[^a-z0-9 ]/g, '');
 ...
 */
function findConditionIdFromAIDetection(detectedCondition: string, conditions: Map<string, string>): string | null {
  ...
  const AI_RESPONSE_MAP: Record<string, string[]> = {
    ...
    'third degree av block': ['CV__conduction_disorders__third_degree_av_block', 'complete heart block'],
  ...
  for (const [condId, condName] of conditions) {
    ...
    const overlap = detectedWords.filter(w => condWords.some(cw => cw.includes(w) || w.includes(cw)));
...
async function main() {
  ...
  for (const [condId, files] of conditionImages) {
    ...
    if (needed <= 0) {
      ...
      console.log(`  [${processedConditions}/${totalConditions}] ${condId}: already has ${existingCount} images, skipping`);
      continue;
    }
    
    console.log(`  [${processedConditions}/${totalConditions}] Processing ${condId} (need ${needed} more images)...`);
      ...
      if (!SKIP_AI) {
        ...
        if (!file.analysis.matchesExpected && file.analysis.confidence > 0.7) {
          ...
          if (newConditionId && newConditionId !== condId) {
            console.log(`  🔀 ${file.conditionName}: Rerouting to ${conditions.get(newConditionId) || newConditionId} (AI: ${detectedCond})`);
            ...
            console.log(`  ℹ️ ${file.conditionName}: AI detected "${detectedCond}" but keeping original classification`);
          }
        }
        
        // Skip images with PROBLEMATIC annotations (not standard labels)
        if (file.analysis.hasAnnotations && file.analysis.annotationType !== 'none' && !FORCE_UPLOAD) {
          ...
          if ((file.analysis.hasSourceText || file.analysis.suggestedCrop) && isSharpAvailable()) {
            console.log(`  📐 ${file.conditionName}: Will crop to remove: ${file.analysis.annotationDetails.join(', ')}`);
            // Continue processing - will crop later
          } else if (file.analysis.annotationType === 'diagnostic_label' || 
                     file.analysis.annotationType === 'answer_revealing') {
            console.log(`  ⚠️ ${file.conditionName}: Skipped (${file.analysis.annotationType}: ${file.analysis.annotationDetails.join(', ')})`);
        ...
        if (file.analysis.quizSuitability === 'unusable') {
          console.log(`  ⚠️ ${file.conditionName}: Skipped (${file.analysis.quizSuitabilityReason})`);
          stats.skipped++;
          continue;
        }
