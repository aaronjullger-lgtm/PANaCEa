...
const SYSTEM_MAP: Record<string, string> = {
  'cardiology': 'CV', 'cardiovascular': 'CV', 'cv': 'CV',
  'pulmonology': 'PULM', 'pulmonary': 'PULM', 'pulm': 'PULM',
  'dermatology': 'DERM', 'derm': 'DERM',
  'musculoskeletal': 'MSK', 'msk': 'MSK', 'ortho': 'MSK',
  'neurology': 'NEURO', 'neuro': 'NEURO',
  'gastroenterology': 'GI', 'gi': 'GI',
  'heent': 'HEENT', 'ent': 'HEENT',
  'infectious': 'ID', 'id': 'ID',
  'hematology': 'HEME', 'heme': 'HEME',
  'endocrinology': 'ENDO', 'endo': 'ENDO',
  'nephrology': 'RENAL', 'renal': 'RENAL',
  'reproductive': 'REPRO', 'repro': 'REPRO', 'obgyn': 'REPRO',
  'psychiatry': 'PSYCH', 'psych': 'PSYCH',
  'urology': 'GU', 'gu': 'GU',
};
...
function extractVisualFindings(buzzwords: string[] | null, modality: string): string[] {
  ...
  const visualKeywords = [
    'rhythm', 'wave', 'interval', 'segment', 'complex', 'pattern',
    'sign', 'finding', 'appearance', 'visible', 'shows', 'demonstrates',
    'lesion', 'opacity', 'density', 'mass', 'shadow', 'enlargement',
    'widened', 'narrowed', 'elevated', 'depressed', 'prolonged',
    'irregular', 'regular', 'absent', 'present', 'classic',
    'pathognomonic', 'characteristic', 'typical', 'diagnostic'
  ];
...
function getModalityInterpretationTips(modality: string, condition: string): string | null {
  const tips: Record<string, string> = {
    ...
    'ultrasound': 'Orient yourself to the probe marker. Identify key landmarks first. Look for echogenicity changes, masses, and fluid collections.',
    'clinical_photo': 'Note the location, distribution, morphology (shape, borders), color, and associated findings. Consider the clinical context.',
    'fundoscopy': 'Systematically examine: Optic disc → Vessels → Macula → Peripheral retina. Note any hemorrhages, exudates, or vascular changes.',
    'microscopy': 'Start with low power to assess overall architecture, then high power for cellular detail. Look for the characteristic pattern or cells.',
  };
...
function detectModality(query: string): string {
  ...
  if (q.includes('skin') || q.includes('rash') || q.includes('lesion') || q.includes('derm')) return 'clinical_photo';
...
async function main() {
  ...
  try {
    switch (command) {
      ...
      case "fetch-one":
        if (!arg1) {
          console.log("Usage: npx tsx scripts/images/bulk-image-fetcher.ts fetch-one <conditionId>");
          break;
        }
      ...
      default:
        ...
        console.log("  npx tsx scripts/images/bulk-image-fetcher.ts preview CV__ecg__atrial_fibrillation");
        console.log("  npx tsx scripts/images/bulk-image-fetcher.ts fetch cardiology 5");
        console.log("  npx tsx scripts/images/bulk-image-fetcher.ts fetch-one CV__ecg__torsades_de_pointes");
    }
  } finally {
