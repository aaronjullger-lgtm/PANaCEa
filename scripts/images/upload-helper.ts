    ...
    const medicalContent = await prisma.medicalContent.findFirst({
      where: {
        OR: [
          ...
          {
            conditionId: {
              contains: condition.name.toLowerCase().replace(/\s+/g, '_'),
              mode: 'insensitive',
            },
          },
...
function buildClinicalContextFromDb(
  ...
  if (dbContent.classicPatient) {
    ...
    try {
      const symptomsArr =
        typeof dbContent.symptoms === 'string' && dbContent.symptoms.startsWith('[')
  ...
  if (dbContent.physicalExam) {
    try {
      const examData =
        typeof dbContent.physicalExam === 'string' && dbContent.physicalExam.startsWith('{')
          ? JSON.parse(dbContent.physicalExam)
          : dbContent.physicalExam;
      if (typeof examData === 'object') {
        additionalFindings = Object.values(examData)
          .slice(0, 3)
          .map((v) => String(v).replace(/\*\*/g, '').substring(0, 80));
      }
    } catch {
...
async function ensureBucketExists(): Promise<boolean> {
  ...
  try {
    ...
    const checkResponse = await fetch(`${SUPABASE_URL}/storage/v1/bucket/${MEDIA_BUCKET}`, {
      headers: {
...
async function uploadSingleImage(
  ...
  try {
    ...
    if (!imageUrl || !category || !correctDiagnosis || distractors.length < 3) {
      return {
        success: false,
        error: 'Missing required fields: imageUrl, category, correctDiagnosis, and 3+ distractors',
      };
    }
    ...
    console.log(
      `📊 Image metadata: ${imageData.metadata.width}x${imageData.metadata.height}, ${(imageData.metadata.fileSize / 1024).toFixed(1)}KB, ${imageData.metadata.mimeType}`
    );
    ...
    const storedUrl = await uploadToSupabase(imageData.buffer, imageData.contentType, storagePath);
    if (!storedUrl) {
    ...
    const clinicalContext = options.clinicalContext
      ? {
          ...
          chiefComplaint:
            options.clinicalContext.chiefComplaint || `Evaluation for ${correctDiagnosis}`,
    ...
    const explanation =
      options.explanation || buildExplanationFromDb(dbContent, correctDiagnosis, modality);
    ...
    const description =
      options.description || buildDescriptionFromDb(dbContent, correctDiagnosis, modality);
...
async function main() {
  ...
  switch (command) {
    case 'upload': {
      ...
      const result = await uploadSingleImage(imageUrl, category, diagnosis, distractors);
    ...
    case 'batch': {
      const [, jsonFile] = args;
      if (!jsonFile) {
        console.log('Usage: npx tsx scripts/images/upload-helper.ts batch <jsonFile>');
        console.log('\nJSON file format:');
        console.log('[');
        console.log('  {');
        ...
        console.log(`    "distractors": ["Atrial Flutter", "MAT", "Sinus Arrhythmia"],`);
    ...
    case 'test': {
      ...
      console.log(
        `  Service Key: ${
          SUPABASE_SERVICE_KEY ? `${SUPABASE_SERVICE_KEY.substring(0, 20)}...` : 'NOT SET'
        }`
      );
      ...
      try {
        ...
        if (listResponse.ok) {
          ...
          const hasMedicalImages = buckets.some((b: any) => b.name === MEDIA_BUCKET);
          if (hasMedicalImages) {
        ...
        if (uploadResponse.ok || uploadResponse.status === 200) {
          ...
          // Clean up test file
          await fetch(`${SUPABASE_URL}/storage/v1/object/${MEDIA_BUCKET}/test-connection.txt`, {
            method: 'DELETE',
            headers: {
              Authorization: `Bearer ${SUPABASE_SERVICE_KEY}`,
              apikey: SUPABASE_SERVICE_KEY,
          ...
          console.log(`   ❌ Upload test failed: ${uploadResponse.status} - ${uploadError}`);
        }
