...
const STANDARD_STRUCTURE = {
  overview: {
    minWords: 50,
    structure: 'Start with a clear definition. Include prevalence if notable. End with clinical significance.',
    example: 'Condition X is a [type] characterized by [key features]. It affects approximately [prevalence]. [Clinical significance].'
  },
  etiology: {
    minWords: 40,
    structure: 'Primary causes first, then contributing factors. Use categories if multiple etiologies.',
    example: '**Primary causes**: [list]. **Contributing factors**: [list]. **Genetic/Environmental**: [if applicable].'
  },
  pathophysiology: {
    minWords: 50,
    structure: 'Step-by-step mechanism. Start with initial insult, progress through cascade, end with clinical manifestation.',
    example: 'Initial [trigger] → [cascade steps] → [end result/clinical presentation].'
  },
  diagnostics: {
    minWords: 50,
    structure: 'Organized by testing sequence: Initial workup → Confirmatory tests → Gold standard. Include sensitivity/specificity if known.',
    example: '**Initial**: [screening tests]. **Confirmatory**: [diagnostic tests]. **Gold standard**: [definitive test].'
  },
  treatment: {
    minItems: 3,
    structure: 'Array format. Order: First-line → Second-line → Adjunctive → Emergency. Include drug classes and specific examples.',
    example: ['First-line: [drug class] (e.g., [specific drug])', 'Second-line: [alternatives]', 'Adjunctive: [supportive care]']
  },
  symptoms: {
    minItems: 4,
    structure: 'Array format. Cardinal symptoms first, then common, then rare but important.',
    example: ['Cardinal symptom 1', 'Cardinal symptom 2', 'Common symptom', 'Rare but significant symptom']
  },
  complications: {
    minItems: 3,
    structure: 'Array format. Life-threatening first, then serious, then manageable.',
    example: ['Life-threatening: [complication]', 'Serious: [complication]', 'Common: [complication]']
  }
};
...
function validateStructure(content: any): {
  ...
  if (content.overview && content.overview !== 'NONE') {
    ...
    if (wordCount < STANDARD_STRUCTURE.overview.minWords) {
      issues.push(`overview too short (${wordCount}w, min: ${STANDARD_STRUCTURE.overview.minWords}w)`);
    }
  }
  ...
  if (content.etiology && content.etiology !== 'NONE') {
    ...
    if (wordCount < STANDARD_STRUCTURE.etiology.minWords) {
      issues.push(`etiology too short (${wordCount}w, min: ${STANDARD_STRUCTURE.etiology.minWords}w)`);
    }
  }
  ...
  if (content.pathophysiology && content.pathophysiology !== 'NONE') {
    ...
    if (wordCount < STANDARD_STRUCTURE.pathophysiology.minWords) {
      issues.push(`pathophysiology too short (${wordCount}w, min: ${STANDARD_STRUCTURE.pathophysiology.minWords}w)`);
    }
  }
  ...
  if (content.diagnostics && content.diagnostics !== 'NONE') {
    ...
    if (wordCount < STANDARD_STRUCTURE.diagnostics.minWords) {
      issues.push(`diagnostics too short (${wordCount}w, min: ${STANDARD_STRUCTURE.diagnostics.minWords}w)`);
    }
  }
  ...
  if (content.symptoms && Array.isArray(content.symptoms)) {
    if (content.symptoms.length < STANDARD_STRUCTURE.symptoms.minItems) {
      issues.push(`symptoms insufficient (${content.symptoms.length} items, min: ${STANDARD_STRUCTURE.symptoms.minItems})`);
    }
  }
  ...
  if (content.complications && Array.isArray(content.complications)) {
    if (content.complications.length < STANDARD_STRUCTURE.complications.minItems) {
      issues.push(`complications insufficient (${content.complications.length} items, min: ${STANDARD_STRUCTURE.complications.minItems})`);
    }
  }
...
async function standardizeFormatting(dryRun: boolean = false, regenerate: boolean = false, targetSystem?: string) {
  console.log('╔═══════════════════════════════════════════════════════════╗');
  ...
  for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
    ...
    if (regenerate) {
      ...
      if (!validation.isValid) {
        console.log(`  ⚠️  ${content.conditionId}: ${validation.issues.length} structural issues`);
        ...
        if (validation.needsRegeneration) {
          ...
          for (const issue of validation.issues) {
            ...
            try {
              if (fieldName === 'overview' || fieldName === 'etiology' || 
                  fieldName === 'pathophysiology' || fieldName === 'diagnostics') {
