...
async function main() {
  ...
  const arrayFields = [
    'testQuestionTips', 'boardYieldFacts', 'commonMistakes', 'aliases',
    'advancedWorkup', 'applicableRules', 'mostDangerous', 'oftenMissed', 
    'reassuringFeatures', 'mnemonics'
  ];
  
  console.log('\n=== Array Fields (empty = 0 items) ===');
  for (const field of arrayFields) {
    const emptyCount = records.filter((r: any) => !r[field] || r[field].length === 0).length;
    const status = emptyCount === records.length ? '⚠️ ALL EMPTY' : emptyCount > 0 ? '⚡ PARTIAL' : '✅ FILLED';
