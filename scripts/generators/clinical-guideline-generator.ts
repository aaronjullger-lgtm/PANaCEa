...
const GUIDELINES_TO_GENERATE = [
  // Cardiovascular
  { name: 'HEART Score', category: 'Cardiovascular', description: 'Chest pain risk stratification for ACS' },
  { name: 'CHA2DS2-VASc Score', category: 'Cardiovascular', description: 'Stroke risk in atrial fibrillation' },
  { name: 'HAS-BLED Score', category: 'Cardiovascular', description: 'Bleeding risk on anticoagulation' },
  { name: 'Framingham Risk Score', category: 'Cardiovascular', description: '10-year cardiovascular risk' },
  { name: 'TIMI Risk Score (STEMI)', category: 'Cardiovascular', description: 'Mortality risk in STEMI' },
  { name: 'TIMI Risk Score (NSTEMI/UA)', category: 'Cardiovascular', description: 'Risk stratification in NSTEMI/UA' },
  { name: 'Duke Criteria', category: 'Cardiovascular', description: 'Infective endocarditis diagnosis' },
  
  // Pulmonary/PE/DVT
  { name: 'Wells Score for PE', category: 'Pulmonary', description: 'Pulmonary embolism probability' },
  { name: 'Wells Score for DVT', category: 'Pulmonary', description: 'Deep vein thrombosis probability' },
  ...
  // Neurology
  { name: 'NIH Stroke Scale (NIHSS)', category: 'Neurology', description: 'Stroke severity assessment' },
  { name: 'ABCD2 Score', category: 'Neurology', description: 'TIA stroke risk prediction' },
  { name: 'Canadian CT Head Rule', category: 'Neurology', description: 'CT indication after minor head injury' },
  { name: 'New Orleans Criteria', category: 'Neurology', description: 'CT indication after minor head injury' },
  { name: 'Glasgow Coma Scale', category: 'Neurology', description: 'Level of consciousness assessment' },
  ...
  { name: 'Glasgow-Blatchford Score', category: 'GI', description: 'Upper GI bleed intervention need' },
  ...
  { name: 'Modified Centor (McIsaac)', category: 'ID', description: 'Strep pharyngitis with age adjustment' },
  ...
  // Syncope/Other
  { name: 'San Francisco Syncope Rule', category: 'Syncope', description: 'Short-term serious outcome risk' },
  ...
  { name: 'Columbia Suicide Severity Rating', category: 'Psychiatry', description: 'Suicide risk assessment' },
  { name: 'CAGE Questionnaire', category: 'Psychiatry', description: 'Alcohol use disorder screening' },
  { name: 'AUDIT-C', category: 'Psychiatry', description: 'Alcohol use screening' },
  
  // Pediatrics
  { name: 'PECARN Head CT Rule', category: 'Pediatrics', description: 'Pediatric head CT after trauma' },
  { name: 'Westley Croup Score', category: 'Pediatrics', description: 'Croup severity assessment' },
  { name: 'Pediatric Early Warning Score', category: 'Pediatrics', description: 'Clinical deterioration prediction' },
  
  // Additional PANCE-Relevant Guidelines
  { name: 'APACHE II Score', category: 'Critical Care', description: 'ICU mortality prediction' },
  { name: 'CHADS2 Score', category: 'Cardiovascular', description: 'Stroke risk in atrial fibrillation (simpler version)' },
  { name: 'Modified Early Warning Score (MEWS)', category: 'Critical Care', description: 'Early detection of clinical deterioration' },
  { name: 'Pittsburgh Knee Rules', category: 'MSK', description: 'X-ray indication for knee injury' },
  { name: 'FAST Exam criteria', category: 'Trauma', description: 'Ultrasound assessment for trauma' },
  ...
  { name: 'STOP-BANG Questionnaire', category: 'Sleep Medicine', description: 'Obstructive sleep apnea screening' },
  { name: 'Epworth Sleepiness Scale', category: 'Sleep Medicine', description: 'Daytime sleepiness assessment' },
  { name: 'PAINAD', category: 'Geriatrics', description: 'Pain assessment in advanced dementia' },
  { name: 'CAM (Confusion Assessment Method)', category: 'Geriatrics', description: 'Delirium screening tool' },
  { name: 'Montreal Cognitive Assessment (MoCA)', category: 'Neurology', description: 'Cognitive impairment screening' },
  { name: 'Mini-Mental State Exam (MMSE)', category: 'Neurology', description: 'Cognitive function screening' },
  { name: 'FOUR Score', category: 'Neurology', description: 'Coma assessment (alternative to GCS)' },
];
...
async function gapAnalysis() {
  ...
  if (missing.length > 0) {
    ...
    const byCategory = missing.reduce((acc, g) => {
  ...
  const additionalSuggestions = [
    { name: 'Revised Cardiac Risk Index (RCRI)', category: 'Cardiovascular', description: 'Perioperative cardiac risk assessment' },
    { name: 'Caprini Score', category: 'Hematology', description: 'VTE risk assessment' },
    { name: 'Padua Prediction Score', category: 'Hematology', description: 'VTE risk in medical patients' },
    { name: 'YEARS Algorithm', category: 'Pulmonary', description: 'PE diagnosis with age-adjusted D-dimer' },
    { name: 'Light\'s Criteria', category: 'Pulmonary', description: 'Pleural effusion classification (exudate vs transudate)' },
    { name: 'PESI (Pulmonary Embolism Severity Index)', category: 'Pulmonary', description: 'PE mortality risk stratification' },
    { name: 'Khorana Score', category: 'Hematology', description: 'Chemotherapy-associated VTE risk' },
    { name: 'HOSPITAL Score', category: 'General Medicine', description: 'Readmission risk prediction' },
    ...
    { name: 'Sgarbossa Criteria', category: 'Cardiovascular', description: 'MI diagnosis with LBBB' },
    { name: 'PECARN Abdominal Trauma', category: 'Trauma', description: 'Pediatric abdominal CT indication after trauma' },
    { name: 'DKA Severity Criteria', category: 'Endocrinology', description: 'Diabetic ketoacidosis severity classification' },
  ];
  
  const notInDatabase = additionalSuggestions.filter(s => 
    !allGuidelines.some(g => namesMatch(g.name, s.name))
  );
  
  // Only auto-add if we have good coverage of the main guidelines
  const coveragePercent = ((GUIDELINES_TO_GENERATE.length - missing.length) / GUIDELINES_TO_GENERATE.length) * 100;
  ...
  if (notInDatabase.length > 0 && coveragePercent >= 90) {
    ...
    for (const suggestion of notInDatabase.slice(0, 5)) { // Add up to 5 at a time
