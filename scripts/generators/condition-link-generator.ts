...
async function generateECGLinks(
  ...
  const prompt = retryCount === 0 ? `You are a medical expert. For the ECG pattern "${ecgPattern.name}" (category: ${ecgPattern.category}), identify which conditions from this list it can help diagnose:
    ...
    for (const item of parsed) {
      ...
      if (condition) {
        links.push({
          ...
          findingDescription: item.findingDescription || `${ecgPattern.name} associated with ${condition.name}`,
...
async function generateImagingLinks(
  ...
  const prompt = retryCount === 0 ? `You are a medical expert. For the imaging study "${imaging.name}" (${imaging.modality}, ${imaging.bodyRegion || 'various'}), identify which conditions from this list it can help diagnose:
    ...
    for (const item of parsed) {
      ...
      if (condition) {
        links.push({
          ...
          findingDescription: item.findingDescription || `${imaging.name} findings in ${condition.name}`,
...
async function analyze() {
  ...
  const [ecgCount, imagingCount, conditionCount, ecgLinkCount, imagingLinkCount] = await Promise.all([
...
async function generateECGConditionLinks() {
  ...
  const existingSet = new Set(existingLinks.map(l => 
    `${l.ecgPatternId}|${l.conditionId}|${l.relationshipType}`
  ));
  ...
  for (const ecg of ecgPatterns) {
    ...
    if (conditionsBySystem['Cardiovascular']) {
      relevantConditions = [...new Set([...relevantConditions, ...conditionsBySystem['Cardiovascular']])];
    }
...
async function generateImagingConditionLinks() {
  ...
  const existingSet = new Set(existingLinks.map(l => 
    `${l.imagingId}|${l.conditionId}|${l.relationshipType}`
  ));
