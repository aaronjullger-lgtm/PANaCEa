  ...
  try {
    ...
    for (const record of incompleteRecords) {
      try {
        ...
        const data = await generateLabTestData(
          record.name,
          record.category,
          record.typicalUse
        );
        ...
        if (!record.conventionalRange) updateData.conventionalRange = ensureString(data.conventionalRange);
        ...
        if (record.fastingRequired === false) updateData.fastingRequired = Boolean(data.fastingRequired);
        ...
        if (!record.specialInstructions) updateData.specialInstructions = ensureString(data.specialInstructions);
        if (!record.interpretationSteps) updateData.interpretationSteps = ensureString(data.interpretationSteps);
