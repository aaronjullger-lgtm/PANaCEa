 ...
 * 
 * Usage: npx ts-node scripts/generators/drug-doctor-fixed.ts --limit=100
...
async function generateDrugEnhancement(drug: {
  ...
  indications: string[];
}): Promise<DrugEnhancement> {
  const prompt = `You are a PA/NP pharmacology educator. Generate comprehensive PANCE-focused content for "${drug.genericName}" (${drug.drugClass.join(', ')}).

CRITICAL: Return ONLY valid JSON. Every field MUST match the specified type exactly.
  ...
  for (const drug of drugs) {
    try {
      ...
      if (!dryRun) {
        ...
        console.log(`  ✓ Enhanced ${drug.genericName}`);
      } else {
        console.log(`  [DRY RUN] Would enhance ${drug.genericName}`);
      }

      // Rate limit - 3 seconds for 2.5-pro (more conservative for higher quality model)
