...
type TreatmentCategory = 'Medical' | 'Surgical' | 'Procedural' | 'Physical Therapy' | 'Lifestyle' | 'Emergency' | 'Supportive' | 'Multimodal';
type TreatmentType = 'First-line' | 'Second-line' | 'Adjunct' | 'Salvage' | 'Prophylaxis' | 'Maintenance';
...
const KNOWN_TREATMENT_PROTOCOLS: TreatmentProtocol[] = [
  // ========== CARDIOLOGY ==========
  {
    ...
    category: 'Emergency',
    type: 'First-line',
    description: 'Time-critical management of ST-elevation myocardial infarction with focus on rapid reperfusion',
    indications: ['STEMI on ECG', 'Chest pain with ST elevations ≥1mm in 2 contiguous leads'],
    contraindications: ['Active bleeding', 'Recent surgery (relative for lytics)', 'Prior intracranial hemorrhage (absolute for lytics)'],
    complications: ['Reperfusion arrhythmias', 'Bleeding (with anticoagulation)', 'Cardiogenic shock', 'Mechanical complications'],
    initialManagement: 'MONA (Morphine, Oxygen if SpO2<90%, Nitroglycerin, Aspirin 325mg) + Dual antiplatelet (P2Y12 inhibitor) + Anticoagulation (Heparin)',
    definitiveTherapy: 'Primary PCI (preferred, door-to-balloon <90 min) OR Fibrinolytics (if PCI unavailable within 120 min)',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['Door-to-balloon <90 minutes', 'Door-to-needle <30 minutes for lytics', 'Aspirin 325mg for ALL ACS'],
    clinicalPearls: ['Time is muscle - every minute counts', 'Do NOT delay PCI for additional testing', 'RV infarct = avoid nitrates, give fluids'],
    alternativeTo: [],
  ...
  },
  {
    ...
    category: 'Medical',
    type: 'First-line',
    description: 'Rate control strategy for atrial fibrillation - preferred initial approach for most patients',
    indications: ['Atrial fibrillation with rapid ventricular response', 'Symptomatic AFib', 'New-onset AFib'],
    contraindications: ['Pre-excitation (WPW) - avoid AV nodal blockers', 'Decompensated heart failure (avoid CCBs)', 'Severe bradycardia'],
    complications: ['Bradycardia', 'Hypotension', 'Heart failure exacerbation'],
    initialManagement: 'Beta-blocker (metoprolol 5mg IV, may repeat) OR Diltiazem (0.25mg/kg IV) for rapid rate control',
    definitiveTherapy: 'Oral beta-blocker or non-DHP CCB for long-term rate control. Target HR <110 at rest.',
    supportiveCare: 'Anticoagulation based on CHA2DS2-VASc score (≥2 men, ≥3 women = anticoagulate)',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['CHA2DS2-VASc determines anticoagulation', 'AVOID nodal blockers in WPW', 'Rate control = first-line for most'],
    clinicalPearls: ['Diltiazem preferred if no HFrEF', 'Beta-blocker preferred if HFrEF', 'Digoxin only as adjunct - slow onset'],
  ...
  },
  {
    ...
    contraindications: ['ACEi/ARB: Bilateral renal artery stenosis, angioedema, pregnancy', 'Beta-blocker: Cardiogenic shock, severe bradycardia', 'MRA: K>5.0, eGFR<30', 'SGLT2i: Type 1 DM'],
    complications: ['Hypotension', 'Hyperkalemia', 'Acute kidney injury', 'Bradycardia'],
    initialManagement: 'Start all four pillars simultaneously or rapidly sequentially: ACEi/ARB/ARNI + Beta-blocker + MRA + SGLT2i',
    definitiveTherapy: 'ARNI (sacubitril/valsartan) preferred over ACEi, Carvedilol/Metoprolol succinate/Bisoprolol, Spironolactone/Eplerenone, Dapagliflozin/Empagliflozin',
    supportiveCare: 'Sodium restriction <2g/day, fluid restriction if hyponatremic, daily weights, diuretics PRN for congestion',
    timing: 'Chronic',
    duration: 'Lifelong',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['Four pillars: ARNI/ACEi + BB + MRA + SGLT2i', 'ARNI preferred over ACEi', 'All four reduce mortality'],
    clinicalPearls: ['Start low, go slow but GO', 'Uptitrate every 2-4 weeks', 'Some hypotension/creatinine rise acceptable'],
    alternativeTo: [],
  ...
  // ========== PULMONOLOGY ==========
  {
    ...
    indications: ['CAP in healthy patient without comorbidities', 'CURB-65 score 0-1', 'No recent antibiotic use'],
    contraindications: ['Allergy to recommended antibiotics', 'Severe CAP requiring hospitalization'],
    complications: ['Treatment failure', 'C. diff colitis', 'Drug allergies'],
    initialManagement: 'Amoxicillin 1g TID x 5 days OR Doxycycline 100mg BID x 5 days OR Azithromycin 500mg day 1, then 250mg days 2-5',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['Amoxicillin or doxycycline first-line for healthy outpatients', '5 days is standard duration', 'Add coverage for atypicals if comorbidities'],
    clinicalPearls: ['Azithromycin resistance increasing - amoxicillin preferred', 'Chest X-ray not required for uncomplicated CAP', 'Switch to oral when improving'],
    alternativeTo: [],
  ...
  },
  {
    ...
    indications: ['Increased dyspnea, sputum volume, or sputum purulence', 'COPD patient with acute worsening'],
    contraindications: ['Steroids: Active untreated infection, uncontrolled diabetes (relative)', 'Antibiotics: Mild exacerbation without purulent sputum'],
    complications: ['Respiratory failure', 'Steroid side effects', 'Antibiotic resistance'],
    initialManagement: 'Nebulized SABA (albuterol) + SAMA (ipratropium) q4-6h, Prednisone 40mg daily x 5 days, Antibiotics if purulent sputum (azithromycin or doxycycline)',
    definitiveTherapy: 'Continue bronchodilators, complete steroid course, adjust maintenance therapy if frequent exacerbations',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['ABCs: Antibiotics (if purulent), Bronchodilators, Corticosteroids', 'Target SpO2 88-92% (avoid hyperoxia)', '5 days steroids is sufficient'],
    clinicalPearls: ['Do NOT give high-flow O2 - lose hypoxic drive', 'Antibiotics only if increased sputum purulence', 'Consider BiPAP if severe'],
    alternativeTo: [],
  ...
  },
  {
    ...
    indications: ['Acute wheezing/dyspnea in known asthmatic', 'Peak flow <80% predicted', 'Accessory muscle use'],
    ...
    initialManagement: 'Albuterol nebulizer (2.5-5mg) or MDI with spacer (4-8 puffs) q20min x 3, Ipratropium 0.5mg nebulized x 1-3 doses, Prednisone 40-60mg PO or methylprednisolone 125mg IV',
    definitiveTherapy: 'Continue bronchodilators as needed, oral steroids x 5 days, step up maintenance therapy',
    supportiveCare: 'Oxygen to maintain SpO2 >92%, continuous monitoring, prepare for intubation if severe',
    timing: 'Acute',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['Albuterol + ipratropium + systemic steroids', 'Steroids take 4-6 hours to work', 'Silent chest = ominous sign'],
    clinicalPearls: ['Magnesium sulfate 2g IV for severe exacerbation', 'Heliox if not responding', 'Avoid sedation'],
    alternativeTo: [],
  ...
  // ========== GI ==========
  {
    ...
    indications: ['Acute pancreatitis (2 of 3: characteristic pain, lipase >3x ULN, imaging findings)'],
    contraindications: [],
    complications: ['Necrotizing pancreatitis', 'Pseudocyst', 'ARDS', 'Multiorgan failure', 'Infected necrosis'],
    initialManagement: 'NPO initially, Aggressive IV fluid resuscitation (LR preferred, 250-500mL/hr initially), Pain control (IV opioids), Antiemetics',
    definitiveTherapy: 'Supportive care. ERCP within 24h if biliary obstruction/cholangitis. Surgery for infected necrosis.',
    supportiveCare: 'Monitor for organ failure (Ranson, APACHE II), early enteral nutrition (within 24-48h) if tolerated, ICU if severe',
    timing: 'Acute',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['Aggressive fluid resuscitation is KEY', 'NPO then early enteral nutrition', 'No prophylactic antibiotics'],
    clinicalPearls: ['LR preferred over NS (less SIRS)', 'Early feeding reduces complications', 'CT not needed for diagnosis if clinical criteria met'],
    alternativeTo: [],
  ...
  },
  {
    ...
    indications: ['H. pylori positive (any test)', 'PUD', 'MALT lymphoma', 'Dyspepsia in endemic areas'],
    ...
    initialManagement: 'Bismuth quadruple therapy (if prior macrolide exposure): PPI BID + Bismuth QID + Metronidazole TID + Tetracycline QID x 14 days',
    definitiveTherapy: 'Clarithromycin triple therapy (if no prior macrolide): PPI BID + Clarithromycin 500mg BID + Amoxicillin 1g BID x 14 days',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['14 days is standard duration', 'Confirm eradication with breath test or stool antigen', 'Bismuth quad if macrolide resistance suspected'],
    clinicalPearls: ['Check for penicillin allergy before amoxicillin', 'Compliance is critical - counsel patient', 'Wait 4 weeks after PPI/antibiotic to retest'],
    alternativeTo: [],
  ...
  // ========== INFECTIOUS DISEASE ==========
  {
    ...
    indications: ['Suspected or confirmed sepsis', 'qSOFA ≥2', 'Septic shock (requiring vasopressors)'],
    contraindications: ['Specific antibiotic allergies (adjust regimen)'],
    complications: ['Multiorgan failure', 'ARDS', 'DIC', 'Death'],
    initialManagement: 'Hour-1 bundle: Lactate, Blood cultures (before abx), Broad-spectrum antibiotics, 30mL/kg crystalloid for hypotension/lactate≥4, Vasopressors if hypotensive after fluids',
    definitiveTherapy: 'Source control (drain abscess, remove line, surgery), Narrow antibiotics based on cultures',
    supportiveCare: 'Central line, arterial line, Foley, ICU admission, stress ulcer prophylaxis, DVT prophylaxis, glucose control',
    timing: 'Acute',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['Hour-1 bundle is critical', 'Blood cultures BEFORE antibiotics (but dont delay abx)', '30mL/kg crystalloid bolus'],
    clinicalPearls: ['Norepinephrine is first-line vasopressor', 'Add vasopressin if still hypotensive', 'Lactate clearance guides resuscitation'],
    alternativeTo: [],
  ...
  },
  {
    ...
    initialManagement: 'Outpatient: Cephalexin 500mg QID x 5-7 days OR Dicloxacillin 500mg QID x 5-7 days. Elevate affected extremity.',
    definitiveTherapy: 'Complete antibiotic course, treat underlying predisposing factors (edema, tinea pedis)',
    supportiveCare: 'Mark borders with pen, elevation, warm compresses, treat tinea pedis if present',
    timing: 'Acute',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['Non-purulent = strep coverage (cephalexin)', 'Purulent = MRSA coverage (TMP-SMX or doxy)', 'Mark borders to monitor spread'],
    clinicalPearls: ['Distinguish from DVT (unilateral leg swelling)', 'Treat tinea pedis to prevent recurrence', 'No MRSA coverage needed for non-purulent'],
    alternativeTo: [],
  ...
  // ========== ENDOCRINE ==========
  {
    ...
    contraindications: ['Hypoglycemia (do not give insulin until glucose addressed)', 'Severe hypokalemia (replete K before insulin)'],
    complications: ['Cerebral edema (especially pediatrics)', 'Hypokalemia', 'Hypoglycemia', 'ARDS'],
    initialManagement: 'NS 1-2L bolus, then 250-500mL/hr. Regular insulin 0.1 units/kg/hr IV drip. K replacement if <5.2. Check glucose hourly, BMP q2-4h.',
    definitiveTherapy: 'Continue insulin drip until anion gap closes. Transition to subcutaneous insulin BEFORE stopping drip (overlap 1-2 hours).',
    supportiveCare: 'Identify precipitant (infection, MI, noncompliance), ICU admission, continuous monitoring',
    timing: 'Acute',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['Fluids first, then insulin', 'Replace K if <5.2 before insulin', 'Goal: Close the gap, not just lower glucose'],
    clinicalPearls: ['Add D5 to fluids when glucose <250', 'Do NOT stop insulin drip until gap closed + SQ given', 'K drops with insulin - monitor closely'],
    alternativeTo: [],
  ...
  },
  {
    ...
    indications: ['Elevated TSH with low free T4', 'Symptomatic hypothyroidism', 'TSH >10 even if asymptomatic'],
    contraindications: ['Untreated adrenal insufficiency (treat cortisol first)', 'Recent MI (start low dose)'],
    complications: ['Overreplacement (hyperthyroidism symptoms)', 'Angina/MI (in cardiac patients)'],
    initialManagement: 'Levothyroxine 1.6 mcg/kg/day (full replacement). Start lower in elderly or cardiac patients (25-50 mcg/day).',
    definitiveTherapy: 'Titrate to TSH goal (0.5-2.5 for most). Recheck TSH in 6-8 weeks after any dose change.',
    supportiveCare: 'Take on empty stomach, 30-60 min before breakfast or at bedtime. Separate from calcium, iron, antacids by 4 hours.',
    timing: 'Chronic',
    duration: 'Lifelong',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['Levothyroxine is first-line', 'Check TSH 6-8 weeks after dose change', 'Take on empty stomach, separate from Ca/Fe'],
    clinicalPearls: ['T3 supplementation generally not recommended', 'Pregnancy requires 25-50% dose increase', 'Generic is acceptable'],
    alternativeTo: [],
  ...
  // ========== NEUROLOGY ==========
  {
    ...
    indications: ['Acute ischemic stroke', 'Within 4.5 hours of symptom onset (or last known well)', 'No contraindications'],
    contraindications: ['Intracranial hemorrhage on CT', 'Recent surgery/trauma', 'Active bleeding', 'BP >185/110 despite treatment', 'INR >1.7', 'Platelet <100k'],
    complications: ['Symptomatic ICH (6%)', 'Angioedema', 'Systemic bleeding'],
    initialManagement: 'STAT non-contrast CT head (rule out bleed), Check glucose, coags, platelets. If eligible: Alteplase 0.9mg/kg (max 90mg), 10% bolus, rest over 60 min.',
    definitiveTherapy: 'Consider thrombectomy if large vessel occlusion (up to 24h in select patients). Admit to stroke unit.',
    supportiveCare: 'BP management (<180/105 after tPA for 24h), NPO until swallow eval, DVT prophylaxis, PT/OT',
    timing: 'Acute',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['4.5-hour window for IV tPA', 'Non-contrast CT to rule out hemorrhage', 'BP must be <185/110 before tPA'],
    clinicalPearls: ['Time is brain - do not delay for MRI', 'Dont lower glucose unless >180', 'Tenecteplase is alternative to alteplase'],
    alternativeTo: [],
  ...
  },
  {
    ...
    initialManagement: 'ABCs, O2, IV access, glucose check. Lorazepam 4mg IV (or 10mg IM midazolam if no IV). May repeat x1 in 5-10 min.',
    definitiveTherapy: 'If seizures continue: Load fosphenytoin 20mg PE/kg OR levetiracetam 60mg/kg OR valproate 40mg/kg. If refractory: anesthetic doses (propofol, midazolam, pentobarbital).',
    supportiveCare: 'Continuous EEG, ICU admission, identify cause (AED levels, electrolytes, tox screen, imaging)',
    timing: 'Acute',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['Lorazepam 4mg IV is first-line', 'Fosphenytoin/levetiracetam/valproate for refractory', 'Check glucose - hypoglycemia causes seizures'],
    clinicalPearls: ['Do NOT wait 5 min if actively seizing in front of you', 'IM midazolam if no IV access', 'Pyridoxine for INH overdose seizures'],
    alternativeTo: [],
  ...
  // ========== PSYCHIATRY ==========
  {
    ...
    indications: ['Major depressive disorder (DSM-5 criteria)', 'PHQ-9 ≥10', 'Significant functional impairment'],
    contraindications: ['SSRI: Concurrent MAOI (14-day washout)', 'Bipolar (can trigger mania)', 'Severe suicidal ideation (consider inpatient)'],
    complications: ['Serotonin syndrome', 'Suicidal ideation (monitor in first weeks)', 'Sexual dysfunction', 'GI upset'],
    initialManagement: 'Start SSRI at low dose: Sertraline 50mg daily OR Escitalopram 10mg daily. Add psychotherapy (CBT or IPT).',
    definitiveTherapy: 'Titrate to therapeutic dose over 4-6 weeks. If no response, switch SSRI or augment. Continue 6-12 months after remission.',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['SSRIs are first-line', 'Takes 2-4 weeks to see effect', 'Monitor for suicidality in first weeks'],
    clinicalPearls: ['Sertraline or escitalopram preferred (fewer interactions)', 'Avoid paroxetine in elderly (anticholinergic)', 'Dont stop abruptly - taper'],
    alternativeTo: [],
  ...
  // ========== MSK ==========
  {
    ...
    indications: ['Acute monoarticular arthritis', 'Monosodium urate crystals on aspirate', 'Classic podagra presentation'],
    contraindications: ['NSAIDs: GI bleeding, CKD, heart failure', 'Colchicine: Severe renal/hepatic impairment', 'Steroids: Active infection'],
    complications: ['GI bleeding (NSAIDs)', 'Diarrhea (colchicine)', 'Hyperglycemia (steroids)'],
    initialManagement: 'NSAIDs (indomethacin 50mg TID or naproxen 500mg BID) x 5-7 days OR Colchicine 1.2mg then 0.6mg in 1 hour (low-dose regimen) OR Prednisone 30-40mg daily x 5 days',
    definitiveTherapy: 'Complete anti-inflammatory course. Do NOT start or stop urate-lowering therapy during flare.',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['NSAIDs, colchicine, or steroids for acute flare', 'Do NOT start allopurinol during flare', 'Low-dose colchicine as effective as high-dose'],
    clinicalPearls: ['Intra-articular steroid injection is excellent option', 'Continuing allopurinol if already on it is OK', 'Joint aspiration rules out septic arthritis'],
    alternativeTo: [],
  ...
  // ========== HEME ==========
  {
    ...
    contraindications: ['Hemochromatosis', 'IV iron: Active infection', 'Oral iron: Unable to tolerate/absorb'],
    complications: ['GI upset (constipation, nausea)', 'Iron overload (with IV)', 'Anaphylaxis (rare with IV)'],
    initialManagement: 'Oral ferrous sulfate 325mg (65mg elemental iron) TID or every other day. Take with vitamin C to enhance absorption. Avoid with antacids/calcium.',
    definitiveTherapy: 'Continue until ferritin >100 and Hgb normalized (usually 3-6 months). IV iron if oral intolerant or malabsorption.',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['Oral iron first-line', 'Every other day dosing may be better absorbed', 'Must identify source (GI malignancy in older adults)'],
    clinicalPearls: ['Ferrous sulfate cheapest and effective', 'GI side effects limit compliance - every other day helps', 'IV iron for IBD, CKD, post-bariatric'],
    alternativeTo: [],
  ...
  // ========== RENAL ==========
  {
    ...
    indications: ['K >6.0 mEq/L', 'ECG changes (peaked T waves, widened QRS)', 'Symptomatic hyperkalemia'],
    contraindications: ['Calcium: Digoxin toxicity (give slowly)', 'Insulin: Hypoglycemia (give with glucose)'],
    complications: ['Arrhythmia', 'Cardiac arrest', 'Hypoglycemia (from insulin)'],
    initialManagement: 'Stabilize: Calcium gluconate 1-2g IV (cardiac membrane). Shift: Insulin 10 units + D50 25g IV, Albuterol nebulized, Sodium bicarb if acidotic.',
    definitiveTherapy: 'Eliminate: Loop diuretics (if volume), Kayexalate (slow), Dialysis (if severe or refractory). Fix underlying cause (stop ACEi, K-sparing diuretics).',
    ...
    isHighYield: true,
    isFirstLine: true,
    boardYieldFacts: ['Calcium stabilizes the heart (does NOT lower K)', 'Insulin + glucose shifts K into cells', 'Dialysis for severe/refractory'],
    clinicalPearls: ['ECG changes = give calcium immediately', 'Kayexalate works slowly (not for acute)', 'Check for hemolysis as lab artifact'],
    alternativeTo: [],
...
async function insertTreatmentProtocol(protocol: TreatmentProtocol): Promise<boolean> {
  try {
    ...
    const boardYieldFacts = ensureArray(protocol.boardYieldFacts, 'Review protocol steps for board questions');
    const clinicalPearls = ensureArray(protocol.clinicalPearls, 'Follow evidence-based guidelines');
    const commonMistakes = ensureArray(protocol.commonMistakes, 'No common mistakes documented for this protocol');
    const alternativeTo = ensureArray(protocol.alternativeTo, 'No specific alternatives - this is the standard approach');
    const advantages = ensureArray(protocol.advantages, 'Standard evidence-based approach');
    const disadvantages = ensureArray(protocol.disadvantages, 'Requires appropriate clinical judgment');
    ...
    await prisma.treatment.upsert({
      ...
      create: {
        ...
        recommendation: protocol.recommendation || 'Follow current clinical guidelines for this condition.',
        indications: ensureArray(protocol.indications, 'No specific indications listed'),
        contraindications: ensureArray(protocol.contraindications, 'No absolute contraindications listed'),
...
async function main() {
  ...
  const conditions = await prisma.condition.findMany({
    where: {
      system: {
        in: ['CV', 'PULM', 'GI', 'NEURO', 'MSK', 'ID', 'ENDO', 'HEME', 'RENAL', 'PSYCH', 'DERM', 'REPRO']
      }
    },
    select: { name: true }
  });
  ...
  const conditionsNeedingProtocols = conditions.filter(c => {
    ...
    return !existingNames.has(protocolName) && !existingNames.has(protocolName2) && !existingNames.has(protocolName3) && !existingNames.has(c.name.toLowerCase());
  });

  console.log(`  Processing ${conditionsNeedingProtocols.length} conditions for treatment protocols\n`);
