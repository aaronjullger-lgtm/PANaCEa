...
async function runGapAnalysis() {
  ...
  if (medicalContentTotal > 0) {
    allGaps['MedicalContent'] = await analyzeTableGaps('MedicalContent', prisma.medicalContent, [
      'overview',
      'pathophysiology',
      'epidemiology',
      'etiology',
  ...
  if (labTestTotal > 0) {
    allGaps['LabTest'] = await analyzeTableGaps('LabTest', prisma.labTest, [
      'conventionalRange',
      'clinicalPearls',
      'boardYieldFacts',
      'commonMistakes',
  ...
  if (drugTotal > 0) {
    allGaps['Drug'] = await analyzeTableGaps('Drug', prisma.drug, [
  ...
  if (questionTotal > 0) {
    allGaps['Question'] = await analyzeTableGaps('Question', prisma.question, [
      'explanation',
      'difficulty',
      'tags',
    ]);
  ...
  console.log('\n─────────────────────────────────────');
  const totalFields =
    emptyFields.length + criticalGaps.length + partialFields.length + completeFields.length;
  ...
  console.log(
    `  ${colors.yellow}◐ Partial (50-99%):${colors.reset} ${partialFields.length} fields`
  );
