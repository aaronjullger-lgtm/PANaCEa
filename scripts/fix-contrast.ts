...
async function main() {
  ...
  const currentAccentOnBg = getContrastRatio(
    darkModeColors['accent-button'],
    darkModeColors['bg-primary']
  );
  const currentTextOnAccent = getContrastRatio(
    darkModeColors['text-inverse'],
    darkModeColors['accent-button']
  );
  ...
  Object.entries(byVariable).forEach(([variable, adjList]) => {
    ...
    if (variable === '--color-accent-button') {
      ...
      const newTextOnAccent = getContrastRatio(
        darkModeColors['text-inverse'],
        darkest.suggestedValue
      );

      console.log(`   New contrast ratios:`);
      console.log(
        `     • On bg-primary: ${newAccentOnBg.toFixed(2)}:1 (${newAccentOnBg >= 4.5 ? '✅' : '❌'} WCAG AA)`
      );
      console.log(
        `     • Text on button: ${newTextOnAccent.toFixed(2)}:1 (${newTextOnAccent >= 4.5 ? '✅' : '❌'} WCAG AA)`
      );
    }
    console.log('');
  });
