...
function calculateMetrics(content: any): ContentMetrics {
  ...
  const requiredFields = [
    'overview', 'etiology', 'epidemiology', 'pathophysiology', 'riskFactors',
    'symptoms', 'physicalExam', 'diagnostics', 'treatment', 'complications',
    'prognosis', 'buzzwords', 'clinical_pearls', 'gold_standard_dx', 'first_line_rx'
  ];
  ...
  const depthScore = Math.round((depthMetrics.reduce((a, b) => a + b, 0) / depthMetrics.length) * 100);
...
function deriveStandards(allMetrics: ContentMetrics[]): QualityStandards {
  ...
  return {
    minOverviewWords: Math.max(50, percentile(allMetrics.map(m => m.overviewWordCount), 0.75)),
    minEtiologyWords: Math.max(40, percentile(allMetrics.map(m => m.etiologyWordCount), 0.75)),
    minPathophysiologyWords: Math.max(50, percentile(allMetrics.map(m => m.pathophysiologyWordCount), 0.75)),
    minDiagnosticsWords: Math.max(50, percentile(allMetrics.map(m => m.diagnosticsWordCount), 0.75)),
    minTreatmentItems: Math.max(3, percentile(allMetrics.map(m => m.treatmentCount), 0.75)),
    minSymptomItems: Math.max(4, percentile(allMetrics.map(m => m.symptomsCount), 0.75)),
    minComplicationItems: Math.max(3, percentile(allMetrics.map(m => m.complicationsCount), 0.75)),
    minBuzzwords: Math.max(4, percentile(allMetrics.map(m => m.buzzwordsCount), 0.75)),
    minPearls: Math.max(3, percentile(allMetrics.map(m => m.pearlsCount), 0.75)),
...
async function assessAndRegenerate(regenerate: boolean = false, targetSystem?: string) {
  ...
  const inadequate = allMetrics.filter(m => 
  ...
  console.log(`   Meeting standards: ${allMetrics.length - inadequate.length} (${Math.round((allMetrics.length - inadequate.length) / allMetrics.length * 100)}%)`);
  console.log(`   Below standards: ${inadequate.length} (${Math.round(inadequate.length / allMetrics.length * 100)}%)\n`);
  ...
  for (let i = 0; i < Math.min(10, sorted.length); i++) {
    const m = sorted[i];
    console.log(`   ${i + 1}. ${m.conditionId} (Score: ${m.overallScore}%, Complete: ${m.completenessScore}%, Depth: ${m.depthScore}%)`);
  }
  ...
  for (const metric of inadequate) {
    try {
      ...
      if (metric.pathophysiologyWordCount < standards.minPathophysiologyWords) weakFields.push('pathophysiology');
      if (metric.diagnosticsWordCount < standards.minDiagnosticsWords) weakFields.push('diagnostics');
      ...
      if (metric.complicationsCount < standards.minComplicationItems) weakFields.push('complications');
...
Return ONLY valid JSON, no markdown wrapper, no explanation.`;
      ...
      regenerated++;
      console.log(`  ✅ ${metric.conditionId}: Enhanced ${Object.keys(improvements).length} fields`);
