...
const SUBCATEGORY_PATTERNS: Record<string, { subcategory: string; parent?: string; keywords: string[] }> = {
  // Cardiovascular - Acute Coronary Syndrome
  'acs': {
    subcategory: 'Acute Coronary Syndrome',
    parent: 'Coronary Artery Disease',
    keywords: ['stemi', 'nstemi', 'unstable angina', 'myocardial infarction', ' mi ', 'acute mi', 'inferior mi', 'anterior mi', 'lateral mi', 'posterior mi']
  },
  
  // Cardiovascular - Cardiomyopathy
  'cardiomyopathy': {
    subcategory: 'Cardiomyopathy',
    keywords: ['dilated cardiomyopathy', 'hypertrophic cardiomyopathy', 'restrictive cardiomyopathy']
  },
  
  // Cardiovascular - Arrhythmias
  'arrhythmia': {
    subcategory: 'Conduction Disorders / Dysrhythmias',
    keywords: ['atrial fibrillation', 'atrial flutter', 'ventricular tachycardia', 'bradycardia', 'tachycardia', 'arrhythmia', 'dysrhythmia', 'block', 'fibrillation', 'brugada', 'long qt', 'wpw', 'wolff-parkinson']
  },
  
  // Cardiovascular - Vascular Disease
  'vascular': {
    subcategory: 'Vascular Disease',
    keywords: ['aneurysm', 'dissection', 'dvt', 'deep venous thrombosis', 'deep vein thrombosis', 'peripheral artery', 'varicose', 'phlebitis', 'arteritis']
  },
  ...
  'valve': {
    ...
    keywords: ['valve', 'valvular', 'stenosis', 'regurgitation', 'prolapse', 'aortic valve', 'mitral valve']
  },
  ...
  'stroke': {
    ...
    keywords: ['stroke', 'cva', 'tia', 'transient ischemic', 'cerebrovascular accident', 'hemorrhage intracranial']
  },
...
function matchPatternSubcategory(conditionName: string, system: string): { 
  ...
  const acronyms = ['stemi', 'nstemi', 'mi', 'pe', 'dvt', 'af', 'vt', 'wpw', 'all', 'aml', 'cll', 'cml', 'ibd', 'copd', 'ckd', 'ards'];
...
function inferSubcategoryFromCurrent(
  ...
  const validBlueprintCategories = [
    'Cardiomyopathy', 'Conduction Disorders / Dysrhythmias', 'Congenital Heart Disease',
    'Coronary Artery Disease', 'Heart Failure', 'Hypertension', 'Hypotension',
    'Lipid Disorders', 'Shock', 'Valvular Disorders', 'Vascular Disease',
    'Acute Coronary Syndrome', 'Traumatic, Infectious, and Inflammatory Heart Conditions',
    'Seizure Disorders', 'Cerebrovascular Disorders', 'Headaches',
    'Obstructive Pulmonary Diseases', 'Infectious Disorders',
    'Hepatic Disorders', 'Biliary Disorders', 'Colorectal Disorders',
...
async function main() {
  ...
  console.log(`  High Confidence: ${stats.highConfidence} (${((stats.highConfidence / conditions.length) * 100).toFixed(1)}%)`);
  console.log(`  Medium Confidence: ${stats.mediumConfidence} (${((stats.mediumConfidence / conditions.length) * 100).toFixed(1)}%)`);
  console.log(`  Low Confidence: ${stats.lowConfidence} (${((stats.lowConfidence / conditions.length) * 100).toFixed(1)}%)`);
  ...
  highConfSamples.forEach(m => {
    ...
    console.log(`    Proposed: ${m.proposedParentCategory ? m.proposedParentCategory + ' → ' : ''}${m.proposedSubcategory}`);
