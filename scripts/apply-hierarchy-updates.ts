...
async function main() {
  ...
  if (!fs.existsSync(mappingsPath)) {
    ...
    console.error('   Run Sprint 3 script first: npx tsx scripts/categorize-existing-conditions.ts');
    process.exit(1);
  }
  ...
  if (changes.length > 0) {
    ...
    changes.slice(0, 10).forEach(change => {
      ...
      console.log(`    Before: ${change.before.parent ? change.before.parent + ' → ' : ''}${change.before.subcategory || 'none'}`);
      console.log(`    After:  ${change.after.parent ? change.after.parent + ' → ' : ''}${change.after.subcategory}\n`);
    });
