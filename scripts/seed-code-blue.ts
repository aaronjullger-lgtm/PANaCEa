...
const CODE_BLUE_QUESTIONS: CodeBlueQuestionSeed[] = [
  ...
  },
  {
    ...
    options: ['One hand on forehead', 'Two fingers on lower sternum', 'Heel of one hand', 'Two hands overlapped'],
    correctIndex: 1,
    explanation: 'Single rescuer infant CPR: two fingers on lower half of sternum, just below nipple line.',
    category: 'BLS'
  },
  {
...
async function seedCodeBlueQuestions() {
  ...
  try {
    ...
    const existingCount = await prisma.aCLSQuestion.count();
    ...
    if (existingCount > 0) {
      ...
      await prisma.aCLSQuestion.deleteMany({});
      console.log('✅ Deleted.\n');
    }
    ...
    for (const q of CODE_BLUE_QUESTIONS) {
      await prisma.aCLSQuestion.create({
        data: {
          question: q.question,
          options: q.options,
