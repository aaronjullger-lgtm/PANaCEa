...
function analyzeColorPairs(colors: Record<string, string>, mode: 'light' | 'dark'): ContrastIssue[] {
...
function printResults(issues: ContrastIssue[], mode: 'light' | 'dark') {
  ...
  if (highIssues.length > 0) {
    ...
    highIssues.forEach(issue => {
      ...
      console.log(`    Ratio: ${issue.ratio.toFixed(2)}:1 (AA: ${issue.meetsAA ? '✓' : '✗'}, AAA: ${issue.meetsAAA ? '✓' : '✗'})`);
  ...
  if (lowIssues.length > 0) {
    ...
    lowIssues.forEach(issue => {
      console.log(`  • ${issue.description}`);
      console.log(`    Ratio: ${issue.ratio.toFixed(2)}:1 (AA: ✓, AAA: ${issue.meetsAAA ? '✓' : '✗'})`);
    });
  }
}
...
async function main() {
  ...
  const highMediumIssues = allIssues.filter(i => i.severity === 'high' || i.severity === 'medium');
