...
async function main() {
  ...
  try {
    ...
    if (anatomyMissingBoardFacts.length > 0 && anatomyMissingBoardFacts.length <= 20) {
      ...
      if (anatomyMissingBoardFacts.length > 10) console.log(`    ... and ${anatomyMissingBoardFacts.length - 10} more`);
    }
    ...
    if (physioMissingBoardFacts.length > 0 && physioMissingBoardFacts.length <= 20) {
      ...
      if (physioMissingBoardFacts.length > 10) console.log(`    ... and ${physioMissingBoardFacts.length - 10} more`);
    }
    ...
    const medicalContentIssues = await prisma.medicalContent.findMany({
      ...
      select: { id: true, conditionId: true, overview: true, symptoms: true, treatment: true, first_line_rx: true, gold_standard_dx: true }
    });
    ...
    if (medicalContentIssues.length > 0) {
      ...
      if (medicalContentIssues.length > 10) console.log(`    ... and ${medicalContentIssues.length - 10} more`);
    }
    ...
    console.log('\n🎯 Priority Actions:');
    if (medicalContentIssues.length > 0) {
      console.log(`  1. Fill ${medicalContentIssues.length} MedicalContent critical fields (HIGHEST PRIORITY)`);
    }
    ...
    if (treatmentMissingDescription.length + treatmentMissingBoardFacts.length > 0) {
      console.log(`  5. Fill ${treatmentMissingDescription.length + treatmentMissingBoardFacts.length} Treatment gaps`);
    }
