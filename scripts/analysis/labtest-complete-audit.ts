...
async function main() {
  ...
  try {
    ...
    // Check each field
    const fields = {
      typicalUse: await prisma.labTest.count({ where: { OR: [{ typicalUse: null }, { typicalUse: '' }] } }),
      conventionalRange: await prisma.labTest.count({ where: { OR: [{ conventionalRange: null }, { conventionalRange: '' }] } }),
      ...
      sampleType: await prisma.labTest.count({ where: { OR: [{ sampleType: null }, { sampleType: '' }] } }),
      collectionTube: await prisma.labTest.count({ where: { OR: [{ collectionTube: null }, { collectionTube: '' }] } }),
      fastingRequired: await prisma.labTest.count({ where: { fastingRequired: false } }), // check how many are unset
      interpretationSteps: await prisma.labTest.count({ where: { OR: [{ interpretationSteps: null }, { interpretationSteps: '' }] } }),
      falsePosNeg: await prisma.labTest.count({ where: { OR: [{ falsePosNeg: null }, { falsePosNeg: '' }] } }),
      turnaroundTime: await prisma.labTest.count({ where: { OR: [{ turnaroundTime: null }, { turnaroundTime: '' }] } }),
      stability: await prisma.labTest.count({ where: { OR: [{ stability: null }, { stability: '' }] } }),
      specialInstructions: await prisma.labTest.count({ where: { OR: [{ specialInstructions: null }, { specialInstructions: '' }] } }),
      // Arrays
      commonAbnormalities: await prisma.labTest.count({ where: { commonAbnormalities: { isEmpty: true } } }),
      increaseIndicates: await prisma.labTest.count({ where: { increaseIndicates: { isEmpty: true } } }),
      decreaseIndicates: await prisma.labTest.count({ where: { decreaseIndicates: { isEmpty: true } } }),
      ...
      interferingFactors: await prisma.labTest.count({ where: { interferingFactors: { isEmpty: true } } }),
      // Board prep
      clinicalPearls: await prisma.labTest.count({ where: { clinicalPearls: { isEmpty: true } } }),
      boardYieldFacts: await prisma.labTest.count({ where: { boardYieldFacts: { isEmpty: true } } }),
      testQuestionTips: await prisma.labTest.count({ where: { testQuestionTips: { isEmpty: true } } }),
    ...
    for (const [field, count] of sortedFields) {
      ...
      console.log(`  ${field.padEnd(22)} ${String(count).padStart(4)}/${total} (${pct.padStart(5)}%) ${bar}`);
    }
    ...
    const commonLabTests = [
      ...
      'Lactate', 'Ammonia', 'Uric Acid',
      'ABG', 'Arterial Blood Gas', 'VBG',
      'CSF Analysis', 'Synovial Fluid Analysis',
      'Pregnancy Test', 'hCG', 'Beta-hCG'
    ];
    
    const missingCommonLabs = commonLabTests.filter(lab => 
      !existingLabNames.has(lab.toLowerCase())
    );
