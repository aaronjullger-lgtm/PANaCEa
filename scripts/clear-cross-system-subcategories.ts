...
const SYSTEM_SUBCATEGORIES: Record<string, string[]> = {
  'CV': [
    'Acute Coronary Syndrome', 'Cardiomyopathy', 'Conduction Disorders / Dysrhythmias',
    'Congenital Heart Disease', 'Coronary Artery Disease', 'Heart Failure',
    'Hypertension', 'Hypotension', 'Lipid Disorders', 'Shock',
    'Traumatic, Infectious, and Inflammatory Heart Conditions', 'Valvular Disorders',
    'Vascular Disease', 'ECG'
  ],
  'PULM': [
    'Infectious Disorders', 'Obstructive Pulmonary Diseases', 'Pneumonias',
    'Restrictive Pulmonary Diseases', 'Vascular Disorders'
  ],
  'NEURO': [
    'Cerebrovascular Disorders', 'Seizure Disorders', 'Headaches',
    'Movement Disorders', 'Neuromuscular Disorders'
  ],
  'GI': [
    'Hepatic Disorders', 'Biliary Disorders', 'Colorectal Disorders',
    'Inflammatory Bowel Disease', 'Esophageal Disorders', 'Gastric Disorders'
  ],
  'PSYCH': [
    'Depressive Disorders', 'Anxiety Disorders', 'Psychotic Disorders',
  ...
  try {
    ...
    for (const condition of conditions) {
      ...
      if (condition.subcategory && !validSubcats.includes(condition.subcategory)) {
        ...
        console.log(`     Valid subcategories for ${condition.system}: ${validSubcats.slice(0, 3).join(', ')}...`);
      }
    }
