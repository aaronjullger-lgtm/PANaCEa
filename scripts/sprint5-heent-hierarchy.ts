...
const HEENT_HIERARCHY: Record<string, { parent: string; keywords: string[] }> = {
  ...
  'Eye - Lacrimal': {
    parent: 'Eye Disorders',
    keywords: ['dacryoadenitis', 'dacryocystitis', 'dacryostenosis', 'keratoconjunctivitis sicca', 'dry eye']
  },
  'Eye - Lid': {
    parent: 'Eye Disorders',
  ...
  'Eye - Retinal': {
    parent: 'Eye Disorders',
    keywords: ['macular degeneration', 'retinal detachment', 'retinopathy', 'retinal vascular occlusion', 'retina']
  },
  ...
  'Ear - Middle': {
    parent: 'Ear Disorders',
    keywords: ['cholesteatoma', 'otitis media', 'otosclerosis', 'tympanic membrane perforation', 'middle ear']
  },
  'Ear - Hearing Impairment': {
    parent: 'Ear Disorders',
    keywords: ['conductive hearing loss', 'sensorineural hearing loss', 'hearing loss', 'hearing impairment']
  },
  'Ear - Other': {
    parent: 'Ear Disorders',
  ...
  'Oropharyngeal - Infectious/Inflammatory': {
    ...
    keywords: ['angioedema', 'aphthous ulcer', 'candidiasis', 'thrush', 'deep neck infection', 
               'dental abscess', 'dental caries', 'epiglottitis', 'gingivitis', 'laryngitis', 
               'peritonsillar abscess', 'pharyngitis', 'tonsillitis', 'throat']
  },
...
async function categorizeHEENTConditions() {
  ...
  for (const condition of heentConditions) {
    ...
    for (const [subcategory, config] of Object.entries(HEENT_HIERARCHY)) {
      for (const keyword of config.keywords) {
        if (nameLower.includes(keyword.toLowerCase())) {
          ...
          if (condition.subcategory !== subcategory || condition.parent_category !== config.parent) {
            updates.push({
              id: condition.id,
              name: condition.name,
    ...
    if (!matched) {
      if (nameLower.includes('nose') || nameLower.includes('sinus') || nameLower.includes('epistaxis') || 
          nameLower.includes('nasal') || nameLower.includes('rhinitis')) {
        ...
        stats.nose++;
        matched = true;
      } else if (nameLower.includes('trauma') || nameLower.includes('fracture') || 
                 nameLower.includes('barotrauma') || nameLower.includes('hyphema') ||
                 nameLower.includes('globe rupture') || nameLower.includes('foreign bod')) {
        ...
        stats.trauma++;
        matched = true;
      } else if (nameLower.includes('neoplasm') || nameLower.includes('tumor') || 
                 nameLower.includes('cancer') || nameLower.includes('carcinoma')) {
  ...
  if (updates.length > 0) {
    ...
    updates.slice(0, 10).forEach(u => {
      ...
      console.log(`    After:  ${u.newParentCategory ? `${u.newParentCategory} → ` : ''}${u.newSubcategory}`);
    });
    console.log();
