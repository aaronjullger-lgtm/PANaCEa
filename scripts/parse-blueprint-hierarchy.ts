...
function parseBlueprint(blueprintPath: string): HierarchyTree {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (!line || line.startsWith('***') || line.startsWith('Here is') || line === '# PANCE Blueprint Master Condition List') {
      continue;
    }
    ...
    if (line.startsWith('## ')) {
      ...
      currentSystem = line.replace(/^##\s+/, '').replace(/\s*\(\d+%\)/, '').trim();
...
function generateReport(tree: HierarchyTree) {
  ...
  if (level2Example) {
    ...
    console.log(`  Example Conditions: ${level2Example.example_conditions.slice(0, 3).join(', ')}\n`);
  }
  ...
  if (level3Example) {
    ...
    console.log(`  Example Conditions: ${level3Example.example_conditions.slice(0, 3).join(', ')}\n`);
  }
