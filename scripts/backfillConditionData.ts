...
function getParentConditionId(meta: ConditionMeta): string | null {
  ...
  if (name.includes('polycystic kidney') && (name.includes('dominant') || name.includes('recessive'))) {
    return buildConditionDefinition({
      system: 'RENAL',
...
async function backfillConditionData() {
  ...
  for (const meta of registryConditions) {
    ...
    if (!existing) {
      ...
      updated++;
    } else if (existing.displayName !== cleanName || !existing.aliases || existing.aliases.length === 0) {
...
async function validateData() {
  ...
  for (const condition of allConditions) {
    if (condition.parentId) {
      ...
      if (!parent) {
        console.error(`❌ Orphaned child: ${condition.name} references non-existent parent ${condition.parentId}`);
        errors++;
      }
    }
  }
