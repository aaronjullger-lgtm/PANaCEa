...
const REQUIRED_SECTIONS = [
  "overview", "diagnostics", "etiologyPathophysiology", "epidemiology",
  "riskFactors", "clinicalPresentation", "symptoms", "examFindings",
  "treatment", "management", "complications", "prognosis"
];
...
function isValidContent(val: any): boolean {
  ...
  if (Array.isArray(val)) {
    ...
    return val.length > 0 && !val.every(s => 
      typeof s === 'string' && (s.includes("--") || s.includes("populated"))
    );
  }
  
  if (typeof val === 'string') {
    const s = val.trim().toLowerCase();
    return s.length > 5 && s !== "--" && !s.includes("to be populated") && !s.includes("content to be generated");
  }
...
function mergeConditions(existing: any, incoming: any): any {
  ...
  for (const section of REQUIRED_SECTIONS) {
    if (isValidContent(incomingContent[section])) {
      ...
      if (!isValidContent(existingSection) || JSON.stringify(existingSection).length < JSON.stringify(incomingContent[section]).length) {
...
async function main() {
  if (!fs.existsSync(INPUT_FILE)) { console.error("File not found"); return; }
  ...
  const incomplete = cleaned.filter(c => 
    REQUIRED_SECTIONS.some(s => !isValidContent(c.content?.[s]))
  ).map(c => c.conditionId);
  ...
  console.log(`⚠️  ${incomplete.length} conditions still missing sections (saved to scrub_report.json).`);
}
