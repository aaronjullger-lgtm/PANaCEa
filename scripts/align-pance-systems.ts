...
async function handlePediatricConditions(dryRun: boolean) {
  ...
  for (const condition of pedsConditions) {
    ...
    if (condition.subcategory?.includes('Cardiac') || condition.name.toLowerCase().includes('heart')) {
      targetSystem = 'Cardiovascular System';
      targetSubcategory = 'Congenital Heart Disease';
    } else if (condition.subcategory?.includes('Oncology') || condition.name.toLowerCase().includes('cancer')) {
...
async function handleOtherConditions(dryRun: boolean) {
  ...
  for (const condition of otherConditions) {
    ...
    if (nameLower.includes('toxicity') || nameLower.includes('poisoning') || 
        nameLower.includes('overdose') || nameLower.includes('ingestion')) {
      targetSystem = 'Gastrointestinal System/Nutrition';
      targetSubcategory = 'Toxicology';
    }
    // Genetic/Congenital Syndromes
    else if (nameLower.includes('syndrome') && (
      nameLower.includes('noonan') || nameLower.includes('prader') || 
      nameLower.includes('angelman') || nameLower.includes('digeorge') ||
      nameLower.includes('fragile x') || nameLower.includes('klinefelter') ||
      nameLower.includes('trisomy') || nameLower.includes('edwards') ||
      nameLower.includes('marfan') || nameLower.includes('ehlers') ||
      nameLower.includes('sturge-weber') || nameLower.includes('tuberous'))) {
    ...
    else if (nameLower.includes('hyaline membrane') || nameLower.includes('meconium') ||
             nameLower.includes('necrotizing enterocolitis') || nameLower.includes('sids')) {
      targetSystem = 'Reproductive System';
      targetSubcategory = 'Neonatal Disorders';
    }
    // GI conditions
    else if (nameLower.includes('colic') || nameLower.includes('encopresis') ||
             nameLower.includes('hirschsprung') || nameLower.includes('dehydration')) {
      targetSystem = 'Gastrointestinal System/Nutrition';
    ...
    else if (nameLower.includes('heat stroke') || nameLower.includes('heat exhaustion') ||
             nameLower.includes('hypothermia') || nameLower.includes('drowning')) {
      targetSystem = 'Cardiovascular System';
      targetSubcategory = 'Environmental Emergencies';
    }
    else if (nameLower.includes('altitude') || nameLower.includes('electrical') ||
...
async function verifySystemAlignment(dryRun: boolean) {
  ...
  console.log(`✅ PANCE-aligned: ${alignedSystems.reduce((sum, s) => sum + s._count, 0)} conditions`);
  
  if (nonAlignedSystems.length > 0) {
    console.log(`⚠️  Non-aligned: ${nonAlignedSystems.reduce((sum, s) => sum + s._count, 0)} conditions`);
  }
