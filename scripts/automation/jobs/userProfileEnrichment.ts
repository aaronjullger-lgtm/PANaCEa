...
async function enrichUserProfile(prisma: PrismaClient, userId: string): Promise<void> {
  ...
  const peakLearningHour = hourlyAccuracies.length > 0
    ? hourlyAccuracies.reduce((best, curr) => 
  ...
  const cognitiveLoadThreshold = sessionPositions.length > 10
    ? calculateCognitiveLoadThreshold(sessionPositions)
    : null;
  
  // Calculate answer change patterns
  const avgAnswerChanges = behaviorMetrics.length > 0
  ...
  } else if (profile.lifetimeAccuracy < 0.60) {
  ...
  const rushingTendency = attempts.length > 0
    ? fastAttempts / attempts.length
    : null;
...
export async function enrichAllUserProfiles(): Promise<void> {
  try {
    ...
    for (let i = 0; i < activeUsers.length; i += batchSize) {
      ...
      await Promise.all(
        batch.map(({ userId }) => enrichUserProfile(prisma, userId))
      );
