 ...
 */
async function getActiveConditions(prisma: PrismaClient, yesterday: Date, today: Date): Promise<string[]> {
...
export async function runContentStatisticsJob(targetDate: Date = new Date()) {
  ...
  try {
    ...
    for (let i = 0; i < activeConditions.length; i += batchSize) {
      ...
      console.log(`   Processed batch ${Math.floor(i / batchSize) + 1}/${Math.ceil(activeConditions.length / batchSize)}`);
    }
    ...
    if (topConditions.length > 0) {
      ...
      topConditions.forEach((stat, index) => {
        ...
        console.log(`      Questions: ${stat.questionsAnswered}, Users: ${stat.uniqueUsers}, Accuracy: ${accuracy}`);
      });
    }
