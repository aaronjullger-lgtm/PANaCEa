...
function parseBlueprint(blueprintPath: string): BlueprintCondition[] {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    if (trimmed.startsWith('## ') && trimmed.includes('System')) {
      currentSystem = trimmed.replace('##', '').trim().replace(/\s*\(\d+%\)/, '');
      currentSubcategory = '';
...
function fuzzyMatch(dbName: string, blueprintName: string): boolean {
  ...
  for (const [abbr, fullForms] of Object.entries(variations)) {
    if ((dbNorm.includes(abbr) || bpNorm.includes(abbr)) &&
...
async function performGapAnalysis() {
  ...
  for (const bpCond of blueprintConditions) {
    const found = dbConditions.some(dbCond => 
      dbCond.system === bpCond.system && 
      fuzzyMatch(dbCond.name, bpCond.condition)
    );
  ...
  const extra = dbConditions.filter(dbCond => 
    !blueprintConditions.some(bpCond => 
      bpCond.system === dbCond.system && 
      fuzzyMatch(dbCond.name, bpCond.condition)
    )
  );
  
  console.log('📊 Gap Analysis Results:\n');
  console.log(`  ✅ Matched: ${matched.length} conditions (${((matched.length / blueprintConditions.length) * 100).toFixed(1)}%)`);
  console.log(`  ❌ Missing: ${missing.length} conditions (${((missing.length / blueprintConditions.length) * 100).toFixed(1)}%)`);
  ...
  const sortedSystems = Object.entries(systemCoverage)
  ...
  for (const [system, conditions] of Object.entries(missingBySystem).sort((a, b) => b[1].length - a[1].length)) {
  ...
  fs.writeFileSync(extraOutputPath, JSON.stringify(extra.map(e => ({
    id: e.id,
    name: e.name,
    system: e.system,
