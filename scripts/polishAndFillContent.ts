...
const sectionPrompts: Record<string, string> = {
  ...
  etiology: "Describe the causes (e.g., infectious agents, genetic mutations, environmental exposures).",
  pathophysiology: "Explain the underlying biological mechanism of disease. Describe the 'how' and 'why'. Include key inflammatory mediators, genetic defects, or physiological disruptions.",
  epidemiology: "Describe who gets this condition (age, sex, demographics, incidence/prevalence).",
  symptoms: "List the hallmark symptoms and patient complaints. Highlight classic presentations (e.g., 'worst headache of life').",
  physicalExam: "List the key physical exam findings. Include vital signs, inspection, palpation, auscultation, and any named signs (e.g., McBurney's point).",
  diagnostics: "Outline the diagnostic approach. Include initial tests (labs, imaging) and the gold standard for diagnosis.",
  treatment: "Outline the management strategy. Structure strictly into: \n1. **Acute/Emergency Management** (if applicable)\n2. **Pharmacotherapy** (First-line, Alternatives)\n3. **Non-pharmacologic/Surgical interventions**.\nMention specific drug classes and names.",
...
async function generateOrPolishContent(condition: string, section: string, currentContent: string | string[] | object | null): Promise<string | string[] | object | null> {
  const isPolish = currentContent !== null && currentContent !== "Content pending generation." && (Array.isArray(currentContent) ? currentContent.length > 0 : true);
  const action = isPolish ? "Rewrite and Improve" : "Generate";
  const isListSection = ['symptoms', 'riskFactors', 'complications', 'physicalExam', 'differentialDiagnosis', 'buzzwords'].includes(section);
