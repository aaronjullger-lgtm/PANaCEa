...
async function assessDatabaseState() {
  ...
  try {
    ...
    console.log(`Usage Rate: ${totalPoolQuestions > 0 ? ((usedPoolQuestions / totalPoolQuestions) * 100).toFixed(1) : 0}%`);
    ...
    if (totalAttempts > 0) {
      ...
      console.log(`Attempts with Telemetry: ${attemptsWithTelemetry} (${((attemptsWithTelemetry / totalAttempts) * 100).toFixed(1)}%)`);
    }
    ...
    if (unusedPoolQuestions > 0) {
      const poolBySystemMap = (await prisma.preGeneratedQuestion.groupBy({
        by: ['system'],
        where: { usedAt: null },
        _count: { id: true }
      })).reduce((acc, { system, _count }) => {
      ...
      if (unusedPoolQuestions > 0) {
        console.log(`  Cardiovascular: ${cardioCount} (${((cardioCount / unusedPoolQuestions) * 100).toFixed(1)}%) - Target: 11%`);
        console.log(`  Pulmonary: ${pulmoCount} (${((pulmoCount / unusedPoolQuestions) * 100).toFixed(1)}%) - Target: 9%`);
        console.log(`  GI/Nutrition: ${giCount} (${((giCount / unusedPoolQuestions) * 100).toFixed(1)}%) - Target: 9%`);
        console.log(`  Musculoskeletal: ${musculoCount} (${((musculoCount / unusedPoolQuestions) * 100).toFixed(1)}%) - Target: 8%`);
      }
    }
    ...
    if (allPass) {
      ...
      console.log('\nNext Steps:');
      console.log('  1. Test session generation: curl localhost:8788/api/questions/session?count=10');
