...
setup('authenticate with Clerk', async ({ page }) => {
  ...
  const isAuthenticated = await page.locator('text=Dashboard, text=Quiz, text=Start Session').first().isVisible({ timeout: 5000 })
    .catch(() => false);
  ...
  if (isAuthenticated) {
    ...
    await page.waitForSelector('text=Dashboard, text=Quiz, text=Start Session', { 
