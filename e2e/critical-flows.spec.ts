...
async function isAuthenticated(page: Page): Promise<boolean> {
  try {
    ...
    const userMenuVisible = await page.locator('[data-testid="user-menu"], [aria-label*="user"], button:has-text("Sign Out")').first().isVisible({ timeout: 2000 });
...
test.describe('Critical User Flows', () => {
  ...
  test.describe('1. Authentication Flow', () => {
    test('should load the application and show auth or dashboard', async ({ page }) => {
      ...
      if (isAuth) {
        ...
        const hasSignIn = await page.locator('text=/sign in|log in|get started/i').first().isVisible({ timeout: 5000 });
    ...
    test('should persist authentication across page reloads', async ({ page }) => {
      ...
      console.log(`✓ Auth state persisted: ${stillAuthenticated ? 'authenticated' : 'not authenticated'}`);
    });
    ...
    test('should show dashboard elements when authenticated', async ({ page }) => {
      ...
      if (isAuth) {
        ...
        const hasModeSelection = await page.locator('text=/mode|drill|study|start/i').first().isVisible({ timeout: 5000 });
        const hasNavigation = await page.locator('nav, [role="navigation"], header').first().isVisible({ timeout: 5000 });
  ...
  test.describe('2. Drill Lifecycle - Rapid Recall', () => {
    test('should navigate to Rapid Recall mode', async ({ page }) => {
      ...
      const hasQuestionUI = await page.locator('text=/question|answer|submit|next/i').first().isVisible({ timeout: 5000 });
    ...
    test('should complete a drill question flow', async ({ page }) => {
      ...
      if (await startButton.isVisible({ timeout: 5000 })) {
        ...
        const hasQuestion = await page.locator('text=/question|answer|choose|select/i').first().isVisible({ timeout: 5000 });
        ...
        if (hasQuestion) {
          ...
          if (await submitButton.isVisible({ timeout: 3000 })) {
            ...
            const hasFeedback = await page.locator('text=/correct|incorrect|right|wrong|explanation|next/i').first().isVisible({ timeout: 3000 });
    ...
    test('should show summary or results after completing questions', async ({ page }) => {
      ...
      console.log(foundSummary ? '✓ Summary view accessible' : 'ℹ Summary view not currently visible (expected if no drills completed)');
    });
  });
  ...
  test.describe('3. Navigation and Content Viewing', () => {
    test('should navigate to library or content section', async ({ page }) => {
      ...
      if (clicked) {
        ...
        const hasContent = await page.locator('text=/condition|topic|system|category/i').first().isVisible({ timeout: 5000 });
    ...
    test('should view details of a content item', async ({ page }) => {
      ...
      if (await libraryLink.isVisible({ timeout: 3000 })) {
        ...
        if (itemClicked) {
          ...
          const hasDetails = await page.locator('text=/symptom|treatment|diagnosis|overview/i').first().isVisible({ timeout: 5000 });
          expect(hasDetails).toBe(true);
          console.log('✓ Detail view loaded with content');
        } else {
          console.log('⊘ No content items found to click - library may be empty or have different structure');
        }
      } else {
  ...
  test.describe('5. Error Handling and Edge Cases', () => {
    test('should handle network errors gracefully', async ({ page }) => {
      ...
      if (await anyButton.isVisible({ timeout: 2000 })) {
        ...
        const hasErrorUI = await page.locator('text=/error|offline|failed|retry/i').first().isVisible({ timeout: 5000 });
