...
test.describe.serial('production smoke: authenticated core study loop', () => {
  test('signs in, studies, persists progress, launches plan context, and searches content', async ({
    ...
    if (!hasAuth) {
      test.skip(
        true,
        'Set CLERK_SECRET_KEY + test email (E2E_CLERK_TEST_EMAIL or PANACEA_E2E_EMAIL) to run authenticated smoke.'
      );
      return;
    }
    ...
    if (!credentials) {
      test.skip(true, 'No Clerk E2E credentials resolved.');
      return;
    }

    const token = await signInWithClerkBackend(page, credentials.email, {
      startPath: '/study',
    });
