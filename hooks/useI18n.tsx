...
export function useI18n() {
  ...
  const changeLocale = useCallback((language: LanguageCode, region: RegionCode) => {
    return i18n.setLocale(language, region);
  }, [i18n]);

  const changeLocaleByString = useCallback((localeString: string) => {
  ...
  const translateMedicalTerm = useCallback((term: string, targetLanguage?: LanguageCode, sourceLanguage?: LanguageCode) => {
    return i18n.translateMedicalTerm(term, targetLanguage, sourceLanguage);
  }, [i18n]);

  const localizeDrugName = useCallback((drugName: string) => {
    return i18n.localizeDrugName(drugName);
  }, [i18n]);

  const formatMedicalValue = useCallback((value: number, unit: string) => {
...
export function useLocaleSwitcher() {
  ...
  const switchLocale = useCallback(async (language: LanguageCode, region: RegionCode) => {
    setIsChanging(true);
    try {
  ...
  const switchLocaleByString = useCallback(async (localeString: string) => {
    setIsChanging(true);
    try {
...
export function withTranslation<T extends object>(
  WrappedComponent: React.ComponentType<T & { t: (key: string, params?: Record<string, string | number>) => string }>
) {
