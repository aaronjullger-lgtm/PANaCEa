...
export function useTypographyAudit(config: TypographyAuditConfig = {}) {
    ...
    elements.forEach((element) => {
      ...
      if (isMobileViewport && fontSizePx < minFontSize) {
        const severity: TypographyIssue['severity'] =
          fontSizePx < 12 ? 'high' : fontSizePx < 14 ? 'medium' : 'low';
  ...
  const applyTypographyFix = useCallback(
    (selector: string, type: TypographyIssue['type'], recommendedValue: string) => {
  ...
  const highlightTypographyIssues = useCallback(
    (highlight: boolean) => {
      if (typeof window === 'undefined') return;
        ...
        issues.forEach((issue) => {
          ...
          if (element instanceof HTMLElement) {
            ...
            element.setAttribute(
              'data-typography-issue',
              `${issue.type}: ${issue.currentValue} → ${issue.recommendedValue}`
            );
          }
        });
      }
