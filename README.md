...
- **Endpoint contracts:** See `docs/api/API_OVERVIEW.md` for current request/response shapes of recently hardened routes (public `GET /api/health` liveness, admin readiness/media, analytics, branches, drills, feedback, graph, library ingestion, medical APIs, push, custom sessions, second-chance reviews, FSRS params, daily plan).
