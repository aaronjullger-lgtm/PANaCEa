...
- **Endpoint contracts:** See `docs/api/API_OVERVIEW.md` for current request/response shapes of recently changed endpoints (`/api/srs/due`, `/api/push/subscribe`, `/api/analytics/soap-note`, `/api/reviews/second-chance`). Admin, OSCE, and other routes are documented in feature-specific guides under `docs/`.
