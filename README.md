...
- **Endpoint contracts:** See `docs/api/API_OVERVIEW.md` for current request/response shapes of actively maintained endpoints, including analytics (`/api/analytics/learner-analysis`, `/api/analytics/readiness-projection`), SRS due queue (`/api/srs/due`), custom sessions (`/api/questions/custom-session`), lab drills (`/api/drills/lab-cases`), feedback (`/api/feedback/submit`), push subscriptions (`/api/push/subscribe`), and second-chance reviews (`/api/reviews/second-chance`).
