...
- **Endpoint contracts:** See `docs/api/API_OVERVIEW.md` for current request/response shapes of actively maintained endpoints (admin, OSCE, agent orchestration — `/api/agents/health`, `/invoke`, `/run`, Agent Protocol `/runs` & `/threads`, MCP `/mcp`, admin tool-loop agents).
