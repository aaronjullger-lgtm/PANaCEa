...
- **Endpoint contracts:** See `docs/api/API_OVERVIEW.md` for request/response shapes of recently changed routes (admin readiness & media, analytics, graph, drills/lab-cases, FSRS params, SRS due, daily plan, push, feedback, custom session, second-chance review). Public liveness: `GET /api/health`; operational diagnostics: admin-only `GET /api/admin/readiness`.
