...
- **Endpoint contracts:** See `docs/api/API_OVERVIEW.md` for current request/response shapes of recently changed endpoints (`/api/health`, `/api/content/library`, `/api/content/systems`, `/api/content/condition/:conditionId/details`, `/api/osce/chat`, `/api/osce/complete`, `/api/questions/due-siblings`, `/api/questions/generate-enhanced`, `/api/questions/pool`, `/api/questions/session`).
