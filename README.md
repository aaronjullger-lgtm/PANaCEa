...
- **Endpoint contracts:** See `docs/api/API_OVERVIEW.md` for request/response shapes of recently changed routes (admin readiness, analytics, branching, drills, feedback, graph, library, push, custom sessions, FSRS params, daily plan). Public liveness: `GET /api/health`; operational diagnostics: `GET /api/admin/readiness` (admin only).
