...
- **Endpoint contracts:** See `docs/api/API_OVERVIEW.md` for current request/response shapes (AI Gateway Gemini proxy, staging/refinery pipeline, agents, admin, OSCE).
...
- **CSP and rate limits:** Security headers (including CSP) are in `public/_headers`. Gemini proxy rate limiting is in `functions/api/_shared/rateLimiter.ts` (applied to `/api/ai/models` and `/api/ai/chat/stream`; legacy `/api/gemini` aliases still recognized by middleware).
