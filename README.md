...
- **Endpoint contracts:** See `docs/api/API_OVERVIEW.md` for request/response shapes of recently changed edge endpoints (public `/api/health` vs admin `/api/admin/readiness`, analytics, drills, custom sessions, FSRS params, daily study plan, push, graph, feedback, branches, and OSCE).
