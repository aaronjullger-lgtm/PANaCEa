...
- **Endpoint contracts:** See `docs/api/API_OVERVIEW.md` for current request/response shapes of actively maintained endpoints (admin readiness/media, analytics, branches, drills/lab-cases, feedback, graph, library contextualization, push subscribe, custom sessions, second-chance reviews, SRS due, FSRS params, daily plan). Public liveness remains at `GET /api/health`; operational diagnostics are admin-only at `GET /api/admin/readiness`.
