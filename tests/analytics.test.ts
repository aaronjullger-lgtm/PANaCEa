...
describe('Analytics Features', () => {
  ...
  describe('Decision Time Analysis', () => {
    it('should calculate average time per system', () => {
      ...
      const cvAvgMs =
        cvRecords.reduce((sum, r) => sum + (r.timeSpentMs || 0), 0) / cvRecords.length;
  ...
  describe('Longitudinal Progress', () => {
    it('should calculate mastery score based on accuracy and volume', () => {
      ...
      const expectedScore1 = Math.min(
        100,
        Math.round(accuracy1 + volumeBonus1 + consistencyBonus1)
      );
      ...
      const expectedScore2 = Math.min(
        100,
        Math.round(accuracy2 + volumeBonus2 + consistencyBonus2)
      );
      ...
      const expectedScore3 = Math.min(
        100,
        Math.round(accuracy3 + volumeBonus3 + consistencyBonus3)
      );
