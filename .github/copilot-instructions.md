...
- Data/registries: Condition metadata and content are database-first (`MedicalContent`, `Condition`). Frontend loads via `services/core/conditionDataLoader.ts`; questions pair `conditionId` with display `condition`. Do not add runtime imports of static `conditionRegistry.ts`.
