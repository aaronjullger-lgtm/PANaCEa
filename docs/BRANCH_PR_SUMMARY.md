...
3. **API validation hardening** — `push/subscribe`, `analytics/soap-note`, `reviews/second-chance`, `feedback/submit`, `questions/custom-session` (bounds; custom-session/soap-note strip unknown client fields to preserve production payloads). *Tests: colocated `.test.ts` (7+7+…).*
...
## Validation (latest — 2026-07-11)
typecheck 0 · typecheck:ci 0 · lint 0 · build ✅ · test:critical 143/143 · **`npm test` 9965 passed / 0 failed / 1 skipped** · **`npm audit --audit-level=moderate` 6** · blocking CI gates ✅ (Build & Test, A11y, All Gates Pass, Agent verify) · **Clerk E2E auth ✅ in CI** (`[ci] E2E env resolved: publishable=yes email=yes secret=yes`; `auth.setup.ts` passes) · Playwright full suite ⚠️ 96 passed / ~69 failed (non-blocking) — failures are API-proxy `ECONNREFUSED` (Playwright job runs Vite only, no Wrangler/Express backend) plus flaky overlay/modal interactions in audit specs · API Smoke (Wrangler) advisory fail · Cloudflare Pages / Prisma Compute external integration · Neon workflow manual-only (Supabase is production DB).

## Unresolved / approval-gated
`npm audit fix` (ERESOLVE — precondition: bump `@cloudflare/workers-types@^5`), dependency majors (react-router-dom/@clerk→js-cookie/nodemailer), RLS DB policy changes, LICENSE choice, secret/anon-key rotation, staging env, production deploy. **E2E auth:** accepts `E2E_CLERK_TEST_EMAIL` or `PANACEA_E2E_EMAIL` + `CLERK_SECRET_KEY` + `VITE_CLERK_PUBLISHABLE_KEY` (backend sign-in via `clerkSetup`). **DB:** Supabase (`DATABASE_URL`); Neon workflow is manual-only/disabled on PRs.
