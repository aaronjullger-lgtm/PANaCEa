...
- **POST /api/feedback/submit:** Body `questionId` (1–200), `flagType` (`incorrect_fact` \| `unclear_question` \| `typo` \| `outdated` \| `other`), `description` (1–2000), optional `questionText` (max 5000), `topic` (max 200), `system` (max 100). Wrapped in `{ body: { … } }`; unknown fields rejected (`.strict()`). Creates `QuestionFlag` record. Used by flag-question flow, `FlagQuestionModal`. Full contract: `docs/api/API_OVERVIEW.md`.
