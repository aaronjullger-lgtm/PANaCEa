...
- **POST /api/feedback/submit:** Body `questionId` (1–200), `flagType` (`incorrect_fact` \| `unclear_question` \| `typo` \| `outdated` \| `other`), `description` (1–2000), optional `questionText` (max 5000), `topic` (max 200), `system` (max 100). `.strict()` rejects unknown fields. Creates `QuestionFlag` record (`201` with `feedbackId`). Used by flag-question flow, FlagQuestionModal. Contract: `docs/api/API_OVERVIEW.md`.
