...
- ✅ **Duplicate semantic cache (FIXED 2026-05-22):** Extracted pure token-match functions to `lib/services/tokenMatchCache.ts` (`tokenizeForCache`, `jaccardSimilarity`, `normalizeMedicalTerms`). `functions/api/_shared/semantic-cache.ts` now imports from the shared module. `lib/services/semanticCacheService.ts` deprecated (only referenced from `_trash/`). File correctly named "tokenMatch" rather than misleading "semantic". All 16 new unit tests pass.
