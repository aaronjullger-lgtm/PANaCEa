...
- `POST /api/feedback/submit` — all persisted free-text fields length-bounded + `.strict()`.
- `GET /api/srs/due` — query param validation (`limit` clamped 1–200, `progressContext`/`context` enum).
