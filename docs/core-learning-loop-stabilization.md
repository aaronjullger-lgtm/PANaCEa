...
**Verification:** `functions/api/srs/{due,submit,submit-compat}.test.ts` → **23 tests pass**. Dashboard response contract (`items`, `totalDue`, `timestamp`; degraded empty payload) pinned in `functions/api/srs/due.test.ts`. Formal wire contract: [`docs/api/API_OVERVIEW.md`](api/API_OVERVIEW.md#get-apisrsdue).
