...
8. TASK-013+ — library-enrichment `.disabled` endpoints, remainder of loading-state rollout (25 non-session/mode files + inline button spinners), and the architecture-level items (NotificationLog schema, runtime-owned push scheduler, Express-to-Edge retirement, study-groups decision). Each still requires reading live callers before committing to a change shape.
...
| Loading-state normalization rollout — remaining scope | First tranche addressed this run (TASK-012: 4 full-page loaders in CoreAdaptiveSession/CramMode/GrandRoundsMode migrated). Remaining: (a) 8 inline button spinners in GrandRoundsMode + PatientEncounterMode need a new canonical `InlineButtonSpinner` primitive first; (b) ~25 other files flagged by the `animate-spin` grep need per-sprint review to keep diffs reviewable. |
