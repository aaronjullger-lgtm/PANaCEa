...
9. TASK-014+ — library-enrichment `.disabled` endpoints, remainder of loading-state rollout (~25 non-session/mode files — needs grep-and-classify per cluster), and the architecture-level items (NotificationLog schema, runtime-owned push scheduler, Express-to-Edge retirement, study-groups decision). Each still requires reading live callers before committing to a change shape.
...
| Loading-state normalization rollout — remaining scope | First tranche addressed in TASK-012 (4 full-page loaders in CoreAdaptiveSession/CramMode/GrandRoundsMode). Second tranche addressed in TASK-013 (new `InlineButtonSpinner` primitive + 8 inline button spinners in GrandRoundsMode + PatientEncounterMode). Remaining: ~25 other files flagged by the `animate-spin` grep (library, dashboard, command palette, my-library, admin, etc.) need a classify-then-migrate pass per cluster to keep diffs reviewable — some are inline-button candidates for the new primitive, some are full-container blocks for `DrillLoadingState` / `ClinicalSkeleton` / `StreamingSkeleton`, and a handful are likely intentional non-loading spinners. TASK-014+. |
