...
- **Sync API:** `GET /api/sync` and `POST /api/sync` in `functions/api/sync.ts`. Authenticated. GET returns `{ success, message, data }` where `data` includes `performanceRecords`, `srsItems`, and `savedQuestions`; POST accepts top-level `userId` plus optional `performanceRecords`/`srsItems`/`savedQuestions` arrays, enforces `userId === auth.userId`, merges to DB, and returns the same `{ success, message, data }` envelope. Uses Prisma, `safePrismaDisconnect`, batched writes, and retry logic for transient failures. Client: `useUserStats` and sync manager call `/api/sync`.
