...
- **Sync API:** `GET /api/sync` and `POST /api/sync` in `functions/api/sync.ts`. Authenticated. Both return `{ success, message, data: { performanceRecords, srsItems, savedQuestions } }`. POST requires `userId` to match auth context and accepts optional `performanceRecords` (max 1000), `srsItems` (max 1000), `savedQuestions` (max 500), then performs batched merge/write. Uses Prisma edge client + `safePrismaDisconnect`. Client: `useUserStats` and sync manager call `/api/sync`.
