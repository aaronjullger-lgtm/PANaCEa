...
- **GET/POST /api/drills/lab-cases** (`functions/api/drills/lab-cases.ts`): Authenticated. GET: `category` (≤50), `limit` (1–100, default 20), `shuffle` (default true) via `labCasesQuerySchema`; returns `{ success, cases, total }` in frontend `LabCase` format. POST: `{ action: "getDiagnoses" }` returns sorted unique diagnoses. DB failures return generic `500` messages (no error leakage). See `docs/api/API_OVERVIEW.md`.
