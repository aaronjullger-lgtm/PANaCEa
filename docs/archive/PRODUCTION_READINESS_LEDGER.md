...
| Deployment target | Cloudflare Pages with Pages Functions and KV namespaces for rate limiting/cache. OSCE Durable Object work is deferred. | `wrangler.toml` had duplicated top-level config; fixed in this batch. On 2026-04-27, removed the OSCE Durable Object binding from the private beta Pages deploy because Pages can bind to an existing DO Worker but cannot deploy that Worker from this project. Production dashboard env parity still needs operator verification. |
...
| AI/tutor/vision | `/api/ai/*`, `/api/tutor/chat`, `/api/agents/*`, `/api/visualizer/*`, `/api/veo/*` | Mostly zod body. | AI envelopes/custom stream; agent routes return `{ ok, data }` with `finalText`, `stopReason`, `tokensUsed`. | User/AI endpoint middleware; `/api/agents/infra-health` is admin-only. | Present through `aiEndpoint` (25 rpm default) and `adminAuthenticatedEndpoint` for infra-health. | Gateway errors handled in several endpoints; agent non-complete runs return 200 with `stopReason`. | Toolkit, tutors, visual tools, agent framework. | Sparse; tool registry integration tests in `lib/services/agents/tools/__tests__/`. |
...
- Primary progress surfaces: `components/dashboard/adaptive/page/DashboardPage.tsx` for `/study`, `pages/ProgressPage.tsx`, `components/analytics/*`, `components/progress-dashboard/*`, `components/navigation/MenuView.tsx`, dashboard widgets for calibration/readiness/review forecast/rolling 360.
...
- Components/hooks: `useTodayPlan`, adaptive `TodayCommandWidget`, `CommandCenterWorkspace`, and study mode contract `study_plan_scheduler` are the primary frontend consumers.
...
- AI/vision/tutor/agents: `/api/agents/run`, `/api/agents/infra-health`, `/api/agents/quality-check`, `/api/agents/coverage-audit`, `/api/agents/verify-condition`, `/api/ai/chat/stream`, `/api/ai/learning/concept-gaps`, `/api/ai/learning/pance-readiness`, `/api/ai/learning/prerequisites`, `/api/ai/learning/profile`, `/api/ai/learning/profile-crud`, `/api/ai/learning/socratic`, `/api/ai/mnemonics/generate`, `/api/ai/generate-mnemonic`, `/api/ai/models`, `/api/ai/sessions/analyze`, `/api/ai/tutor/chat`, `/api/ai/vision/analyze`, `/api/ai/vision/analyze-3d`, `/api/ai/vision/grade-spatial`, `/api/tutor/chat`, `/api/visualizer/edit`, `/api/visualizer/generate`, `/api/veo/generate`, `/api/veo/status`.
...
- OSCE/patient encounter route is not production-ready: new intent/patient/evaluate endpoints are deterministic mocks, migration is unverified, and the Durable Object Worker must be deployed separately before any Pages binding is re-enabled.
...
- 2026-04-27: Removed the deferred `OsceSessionDO` Pages binding from `wrangler.toml` after Cloudflare compiled Functions and published assets but failed at Function publish with an internal error. OSCE remains outside the private beta launch surface.
...
- 2026-04-26: Filtered typecheck found no errors for this study-plan batch. The legacy today-plan card and old dashboard page were later retired in favor of the adaptive Today command.
