...
1. **[API Overview](api/API_OVERVIEW.md)** - Current request/response contracts for sync, authoring, admin health/coverage, cron, and user readiness endpoints
...
- **[API Overview](api/API_OVERVIEW.md)** - Updated contracts for `/api/sync`, `/api/authors/*`, `/api/admin/blueprint-coverage*`, `/api/admin/health-report*`, `/api/cron/*`, `/api/questions/{questionId}/context`, and `/api/users/me/*`
