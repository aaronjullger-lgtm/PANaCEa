...
1. **[API Overview](api/API_OVERVIEW.md)** - Request/response contracts for recently hardened routes
...
- **[API Overview](api/API_OVERVIEW.md)** - Request/response contracts for recently hardened routes (admin readiness & media, analytics, branches, drills, feedback, graph, library ingestion, medical APIs, push, custom sessions, second-chance reviews, FSRS params, daily plan). Public liveness: `GET /api/health`; operational diagnostics: `GET /api/admin/readiness`.
