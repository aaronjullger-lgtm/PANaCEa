...
1. **[API Overview](api/API_OVERVIEW.md)** - Current request/response contracts for updated endpoints (agents, conditions, cron, question pool)
...
- **[API Overview](api/API_OVERVIEW.md)** - Updated endpoint contracts for agents (`/api/agents/health`, `/api/agents/invoke`, `/api/agents/invoke/stream`), conditions (`/api/conditions/high-yield`, `/api/conditions/illness-script`, `/api/conditions/pearls`), cron (`/api/cron/agent-health-check`, `/api/cron/d1-cache-purge`), and question pool (`/api/questions/pool`)
