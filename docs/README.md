...
- **[API Overview](api/API_OVERVIEW.md)** - Updated endpoint contracts for validation-hardened routes (`/api/analytics/soap-note`, `/api/drills/lab-cases`, `/api/feedback/submit`, `/api/push/subscribe`, `/api/questions/custom-session`, `/api/reviews/second-chance`, `/api/srs/due`)
