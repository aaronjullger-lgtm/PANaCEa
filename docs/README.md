...
1. **[API Overview](api/API_OVERVIEW.md)** - Request/response contracts for hardened routes (`/api/analytics/soap-note`, `/api/push/subscribe`, `/api/reviews/second-chance`, `/api/srs/due`) and admin/OSCE endpoints
...
- **[API Overview](api/API_OVERVIEW.md)** - Endpoint contracts for hardened and actively maintained routes: `/api/analytics/soap-note`, `/api/push/subscribe`, `/api/reviews/second-chance`, `/api/srs/due`, plus admin and OSCE endpoints
