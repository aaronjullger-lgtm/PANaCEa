...
1. **[API Overview](api/API_OVERVIEW.md)** - Current request/response contracts for updated endpoints (OSCE Live engine, enhanced question generation, reflection, agent invoke)
...
- **[API Overview](api/API_OVERVIEW.md)** - Updated endpoint contracts for OSCE Live (`/api/osce/live-engine`), enhanced question generation (`/api/questions/generate-enhanced`), session reflection (`/api/reflection`), and encounter agent dispatch (`/api/agents/invoke`)
