...
- 2026-08-01 implicit-confidence compliance (user directive: no confidence self-rating buttons — confidence must be derived implicitly from behavioral cues and collected data): UI layer audited clean — no rating controls anywhere in `components/`; existing regression guards (`ClinicalInterface.test.tsx`, `CommandCenterWorkspace.test.tsx`) assert absence of confidence rating UI; `lib/api/schemas/drills.ts:139` already documents self-reported confidence as intentionally excluded. Fixed + committed: (a) `functions/api/reflection.ts` — schema previously REQUIRED a 1-5 `confidenceRating` that no frontend caller ever sends (every reflection save was failing zod validation); schema now excludes it and persists `0` (unknown) sentinel — commit `355b0362`; (b) `services/analytics/researchBackedAnalytics.ts` — advice copy instructed users to "rate your confidence"; reworded to implicit framing — commit `a46936d0`. Removed (2026-08-01 cont.): `lib/services/gradeModulationCoordinator.ts` `userConfidenceRating?` optional param — dormant legacy, only tests populated it, no production caller; field deleted, `modulateGrade` now hardcodes `confidenceCategory: 'unknown'` (zero confidence delta), 3 test files updated (7 fixtures removed, expectations recomputed), 128/128 tests pass; `mapConfidenceCategory`/`calculateConfidenceDelta` retained as pure utilities for future implicit-confidence inference. Committed in `f55dc40b` (with targeted-conditions widget work). Kept as-is: `lib/services/pedagogicalLlmService.ts` tutor prompt "How confident are you?" — verbal JOL elicitation, not a rating control, no rating data collected.
