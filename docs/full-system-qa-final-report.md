...
Frontend-facing endpoints validated by the mocked automated suite, incl. `/api/srs/due` (stable shape + resilient empty-state, never 500), drills submit-review, questions attempt, dashboard stats/review-queue, push subscribe, second-chance, soap-note, feedback/submit (schemas `.strict()` + bounds). Contracts documented in `docs/api/API_OVERVIEW.md`. No stack traces / secrets in responses (secureLogger + structured errors). 0 failures.
