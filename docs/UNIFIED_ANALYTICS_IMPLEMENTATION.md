...
export function aggregateSession(
  ...
  const accuracyByQuarter = quarters.map(q => 
    q.filter(a => a.isCorrect).length / q.length
  ) as [number, number, number, number];
  
  // Detect trend
  const slope = linearRegression(accuracyByQuarter);
  const accuracyTrend = slope > 0.05 ? 'improving' 
  ...
  const avgConfidence = metrics.reduce((s, m) => s + (m.implicitConfidence ?? 0.5), 0) / metrics.length;
  const avgAccuracy = attempts.filter(a => a.isCorrect).length / n;
  const bias = avgConfidence - avgAccuracy;
  
  const calibrationState = bias > 0.1 ? 'overconfident' 
