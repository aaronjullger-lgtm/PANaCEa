...
const result = await getQuestionsWithNoRepeat('user-123', {
  system: 'CV',
  difficulty: 'medium'
}, 10);
...
const seed = await createQuestionSeed({
  ...
  template: 'A {{Age}}-year-old presents with RLQ pain. The presentation is {{Presentation}}. Vital signs show {{Vitals}} hemodynamics. What is the most likely diagnosis?',
...
const pearl = await createClinicalPearl(
  ...
  { conditionId: 'mi', system: 'CV', difficulty: 'medium' }
);
