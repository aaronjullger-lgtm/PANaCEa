...
export async function onRequestGet(context: any) {
  ...
  try {
    ...
    let userConfusions: Array<{ real: string; mistaken: string; count: number }> = [];
    
    if (userId) {
      // Get user's mastery data (FSRS)
      ...
      // Get confusion patterns involving this condition
      ...
      // Add confusion-based suggestions (CRITICAL priority)
    ...
    // 2. Get direct differential relations from ConditionRelation
    const directRelations = await prisma.conditionRelation.findMany({
      where: {
        OR: [
          ...
          { conditionId2: condition.conditionId, relationType: 'differential', bidirectional: true },
        ],
      },
      include: {
        Condition1: {
    ...
    directRelations.forEach(rel => {
      const otherCondition = rel.conditionId1 === condition.conditionId 
        ? rel.Condition2 
        : rel.Condition1;
    ...
    const complications = await prisma.conditionRelation.findMany({
      where: {
        conditionId1: condition.conditionId,
        relationType: 'complication',
      },
      include: {
        Condition2: {
          select: { id: true, name: true, system: true },
    ...
    // Generate study recommendation based on suggestions
...
function generateStudyRecommendation(
  ...
  if (criticalSuggestions.length > 0) {
    return `PRIORITY: Focus on distinguishing ${condition.condition} from ${criticalSuggestions[0].conditionName}. You frequently confuse these conditions.`;
  }
  ...
  if (highYieldSuggestions.length > 0) {
    return `Study ${condition.condition} alongside these high-yield differentials: ${highYieldSuggestions.slice(0, 3).map(s => s.conditionName).join(', ')}.`;
  }
