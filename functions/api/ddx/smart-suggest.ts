...
interface SmartSuggestion {
  ...
  context: { type: 'weakness' | 'confusion' | 'similar' | 'complication' | 'mastery_gap'; details: string; actionable: string; };
...
export const onRequestGet = publicEndpoint(SmartSuggestSchema, async (context) => {
  ...
  try {
    ...
    const condition = await prisma.medicalContent.findUnique({
      ...
      select: { id: true, conditionId: true, condition: true, system: true, subcategory: true, buzzwords: true, symptoms: true, gold_standard_dx: true, first_line_rx: true, pance_yield: true, classic_patient: true, riskFactors: true },
    });
    ...
    if (userId) {
      ...
      const confusions = await prisma.confusionPair.findMany({
        where: { userId, OR: [{ realConditionId: conditionId }, { mistakenForId: conditionId }], count: { gte: 2 } },
        include: { RealCondition: { select: { id: true, name: true } }, MistakenCondition: { select: { id: true, name: true } } },
      ...
      confusions.forEach((cp) => {
        ...
        if (otherCondition) {
          suggestions.push({
            conditionId: otherCondition.id, conditionName: otherCondition.name, system: condition.system,
            reason: `You've confused ${condition.condition} with this ${cp.count} times`,
            priority: cp.count >= 5 ? 'critical' : 'high',
            context: { type: 'confusion', details: `${cp.count} confusion events recorded`, actionable: 'Review distinguishing features side-by-side' },
            relatedConditionId: conditionId, panceYield: condition.pance_yield || undefined,
          });
        }
      });
    }
    ...
    const directRelations = await prisma.conditionRelation.findMany({
      where: { OR: [{ conditionId1: condition.conditionId, relationType: 'differential' }, { conditionId2: condition.conditionId, relationType: 'differential', bidirectional: true }] },
      include: { Condition1: { select: { id: true, name: true, system: true } }, Condition2: { select: { id: true, name: true, system: true } } },
      take: 10,
    });
    ...
    directRelations.forEach((rel) => {
      ...
      if (otherCondition && !suggestions.some((s) => s.conditionId === otherCondition.id)) {
        suggestions.push({
          conditionId: otherCondition.id, conditionName: otherCondition.name, system: otherCondition.system,
          reason: 'Direct differential diagnosis relationship', priority: 'high',
          context: { type: 'similar', details: rel.clinicalContext || 'Commonly confused on exams', actionable: 'Learn key distinguishing features' },
      ...
      similarByBuzzword.forEach((similar) => {
        if (!suggestions.some((s) => s.conditionId === similar.id)) {
          ...
          suggestions.push({
            ...
            context: { type: 'similar', details: 'Overlapping buzzwords indicate similar presentations', actionable: 'Focus on unique buzzwords for each condition' },
            relatedConditionId: conditionId, panceYield: similar.pance_yield || undefined,
          });
        }
      });
    }
    ...
    complications.forEach((rel) => {
      if (rel.Condition2 && !suggestions.some((s) => s.conditionId === rel.Condition2.id)) {
        suggestions.push({
          ...
          context: { type: 'complication', details: 'Understanding complications improves clinical reasoning', actionable: 'Learn when this complication develops' },
          relatedConditionId: conditionId,
        });
      }
    });
    ...
    if (userId) {
      ...
      sameSystemConditions.forEach((ssc) => {
        ...
        if (isWeakArea && !suggestions.some((s) => s.conditionId === ssc.id)) {
          suggestions.push({
            conditionId: ssc.id, conditionName: ssc.condition, system: ssc.system,
            reason: `Low mastery in same system (${condition.system})`,
            priority: ssc.pance_yield && ssc.pance_yield >= 3 ? 'high' : 'low',
            context: { type: 'mastery_gap', details: mastery ? `Stability: ${mastery.stability.toFixed(1)}` : 'Not yet studied', actionable: 'Review this high-yield condition in the same system' },
            panceYield: ssc.pance_yield || undefined, masteryLevel: mastery ? getMasteryLevel(mastery) : 'new',
          });
        }
      });
    }
    ...
    logger.info('Generated smart suggestions', { conditionId, userId: userId || 'anonymous', count: finalSuggestions.length });

    return {
      data: {
        targetCondition: { id: condition.id, name: condition.condition, system: condition.system, panceYield: condition.pance_yield },
        suggestions: finalSuggestions, studyRecommendation,
        summary: {
          totalSuggestions: finalSuggestions.length,
          criticalCount: finalSuggestions.filter((s) => s.priority === 'critical').length,
          confusionBasedCount: finalSuggestions.filter((s) => s.context.type === 'confusion').length,
...
function getMasteryLevel(mastery: { stability: number; difficulty: number; state: number }): string {
...
function generateStudyRecommendation(suggestions: SmartSuggestion[], condition: any): string {
  ...
  if (highYieldSuggestions.length > 0) return `Study ${condition.condition} alongside these high-yield differentials: ${highYieldSuggestions.slice(0, 3).map((s) => s.conditionName).join(', ')}.`;
