...
async function getSystemBreakdown(
  ...
  const correctMap = new Map<string | null, number>(
    correctBySystem.map((c: { system: string | null; _count: { id: number } }) => [
      c.system,
      c._count.id,
    ])
  );
  const attemptsMap = new Map<string | null, number>(
    attemptsBySystem.map((a: { system: string | null; _count: { id: number } }) => [
      a.system,
      a._count.id,
    ])
  );

  const systems: SystemStats[] = questionsBySystem.map(
    (qs: { system: string; _count: { id: number } }) => {
      ...
      const accuracy = totalAttempts > 0 ? (correctAttempts / totalAttempts) * 100 : 0;
      const coverageScore =
        totalAttempts > 0 ? Math.min(100, (totalAttempts / questionCount) * 10) : 0;
...
async function getQualityMetrics(
  ...
  const topReasons = flagsByType
    .map((f: { flagType: string; _count: { id: number } }) => ({
      reason: f.flagType,
      count: f._count.id,
    }))
    .sort(
      (a: { reason: string; count: number }, b: { reason: string; count: number }) =>
        b.count - a.count
    )
    .slice(0, 5);
...
async function getQuestionDetails(
  ...
  const timesMs = attempts
    .filter((a: { timeSpentMs: number | null }) => a.timeSpentMs)
    .map((a: { timeSpentMs: number | null }) => a.timeSpentMs!);
  const avgTimeMs =
    timesMs.length > 0 ? timesMs.reduce((a: number, b: number) => a + b, 0) / timesMs.length : 0;
  const difficultyRating: 'easy' | 'medium' | 'hard' =
    accuracy >= 80 ? 'easy' : accuracy < 50 ? 'hard' : 'medium';
  ...
  return {
    ...
    recentAttempts: attempts
      .slice(0, 10)
      .map((a: { wasCorrect: boolean; timeSpentMs: number | null; createdAt: Date }) => ({
...
async function getTopQuestions(
  ...
  const correctMap = new Map<string, number>(
    correctCounts.map((c: { questionId: string; _count: { id: number } }) => [
      c.questionId,
      c._count.id,
    ])
  );
