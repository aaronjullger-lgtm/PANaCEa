...
import {
  ...
  type CRAGResult,
} from '@/lib/services/search/correctiveRag';
    ...
    try {
      ...
      if (mode === 'hybrid') {
        ...
        if (enableCRAG && hybridMapped.length > 0) {
          const { filtered, cragMeta } = await applyCRAG(
            query,
            hybridMapped as Array<Record<string, unknown> & { id: string; similarity: number }>
          );
          return {
            data: {
              results: filtered,
        ...
        return {
          data: {
            results: hybridMapped,
            count: hybridMapped.length,
            mode: 'hybrid',
            queryComplexity: complexity,
          },
          headers: { 'Cache-Control': 'private, max-age=60' },
      ...
      if (enableCRAG && results.length > 0) {
        const { filtered, cragMeta } = await applyCRAG(
          query,
          results as Array<Record<string, unknown> & { id: string; similarity: number }>
        );
        return {
