...
export const onRequestGet = publicEndpoint(AllContentSchema, async (context) => {
  const { env } = context;
  ...
  try {
    const allContent = await prisma.medicalContent.findMany({
      ...
      select: {
        conditionId: true, condition: true, system: true, subcategory: true,
        canonicalName: true, buzzwords: true, overview: true, pance_yield: true, classic_patient: true,
      },
    ...
    for (const item of allContent) {
      contentMap[item.conditionId] = {
        conditionId: item.conditionId, condition: item.condition, system: item.system,
        subcategory: item.subcategory, canonicalName: item.canonicalName,
        buzzwords: item.buzzwords && (item.buzzwords as any[]).length > 0 ? item.buzzwords : undefined,
        overview: item.overview, classic_patient: item.classic_patient, pance_yield: item.pance_yield,
        _isSummary: true,
      };
    }
    ...
    logger.error('Error fetching all content', { error: error instanceof Error ? error.message : String(error) });
