...
export const onRequestGet = authenticatedEndpoint(StructuredSchema, async (context) => {
  ...
  try {
    ...
    if (!row) {
      return new Response(
        JSON.stringify({ error: 'Condition not found', identifier }),
...
Output valid JSON only, no markdown.`;
    ...
    if (!res.ok) {
      ...
      logger.warn('Gemini structured parse failed', { status: res.status, text: text.slice(0, 200) });
      return new Response(
        JSON.stringify({
          data: {
            clinical_pearls: [],
    ...
    try {
      ...
      parsed = {
        ...
        gold_standard:
          typeof decoded.gold_standard === 'string' ? decoded.gold_standard : parsed.gold_standard,
    ...
    return new Response(
      JSON.stringify({ data: parsed, source: 'gemini' }),
      { status: 200, headers: { 'Content-Type': 'application/json' } }
    );
  } catch (error) {
