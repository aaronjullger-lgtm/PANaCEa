...
export async function onRequestGet(context: { request: Request; env: Env }) {
  ...
  try {
    ...
    if (weakAreas.length > 2) {
      recommendations.push(`Priority focus: ${weakAreas[0].system} (${weakAreas[0].accuracy}% accuracy)`);
    }

    if (trendDirection === 'declining') {
      recommendations.push('Your accuracy is declining - consider taking a break or reviewing study methods');
    }

    // Time optimization
    const avgTimePerQuestion = records.reduce((sum, r) => sum + Number(r.timeSpent), 0) / records.length;
    const timeOptimization = avgTimePerQuestion > 120 
    ...
    return createSuccessResponse({
      insights: {
        ...
        systemBreakdown: Object.entries(systemPerformance).map(([system, data]) => ({
          system,
          total: data.total,
