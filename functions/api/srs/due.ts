...
function taskKey(
  item: Pick<DueItem, 'conditionId' | 'progressContext' | 'taskType'>
): string | null {
...
function suppressDuplicateDueRows(params: {
  ...
  const specificConditionKeys = new Set(
    [...cardItems, ...keptTopicItems].map(conditionKey).filter((key): key is string => Boolean(key))
  );
  ...
  return {
    ...
    suppressedDuplicates:
      cardItems.length + topicItems.length + conditionItems.length - items.length,
  };
}
      ...
      const cardItems: DueItem[] = cardProgress
        ...
        .map((item) => {
          ...
          return {
            ...
            questionIdentityId: item.questionIdentityId ?? null,
