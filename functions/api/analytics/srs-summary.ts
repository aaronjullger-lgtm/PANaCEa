...
export async function onRequestGet(context: { request: Request; env: CloudflareEnv }): Promise<Response> {
  ...
  try {
    ...
    const recentReviews = progressData.filter(card => 
    ...
    const systemBreakdown = Object.entries(systemMap)
      .map(([system, data]) => ({
        ...
        avgStability: data.cardCount > 0 ? Math.round((data.totalStability / data.cardCount) * 10) / 10 : 0,
        avgDifficulty: data.cardCount > 0 ? Math.round((data.totalDifficulty / data.cardCount) * 10) / 10 : 5,
    ...
    console.error('[SRS Summary API] Error:', error);
    return new Response(JSON.stringify({ error: 'Failed to fetch SRS analytics' }), {
      status: 500,
      headers: { 'Content-Type': 'application/json' },
    });
  } finally {
