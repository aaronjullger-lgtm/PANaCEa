...
export async function onRequestGet(context: any) {
  ...
  try {
    ...
    const avgQualityScore = questions.reduce((sum, q) => sum + (q.qualityScore ?? 0), 0) / questions.length || 0;
    const avgFlagRate = questions.reduce((sum, q) => sum + (q.flagRate ?? 0), 0) / questions.length || 0;
    ...
    const overallAccuracy = totalServed > 0 ? (totalCorrect / (totalCorrect + questions.reduce((sum, q) => sum + q.timesIncorrect, 0))) : 0;
    ...
    console.error('Error fetching question quality analytics:', error);
    return new Response(JSON.stringify({ 
