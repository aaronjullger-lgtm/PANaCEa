...
export const onRequestGet = adminEndpoint(questionQualityQuerySchema, async (context) => {
  ...
  try {
    ...
    const avgQualityScore =
      questions.reduce((sum: number, q: (typeof questions)[0]) => sum + (q.qualityScore ?? 0), 0) /
        questions.length || 0;
    const avgFlagRate =
      questions.reduce((sum: number, q: (typeof questions)[0]) => sum + (q.flagRate ?? 0), 0) /
        questions.length || 0;
    const totalServed = questions.reduce(
      (sum: number, q: (typeof questions)[0]) => sum + q.timesServed,
      0
    );
    const totalCorrect = questions.reduce(
      (sum: number, q: (typeof questions)[0]) => sum + q.timesCorrect,
      0
    );
    const overallAccuracy =
      totalServed > 0
        ? totalCorrect /
          (totalCorrect +
            questions.reduce((sum: number, q: (typeof questions)[0]) => sum + q.timesIncorrect, 0))
        : 0;
