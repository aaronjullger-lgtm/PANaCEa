...
/** Critic (Gemini 1.5 Pro) rubric: strict quality score 0-100 and feedback for aiGrade */
const CRITIC_RUBRIC = `
You are a strict medical education critic. Score this question 0-100.
Rubric:
- Is the correct answer clearly correct? (weight 25)
- Are distractors C and D plausible but clearly wrong? (weight 25)
- Is the vignette unambiguous (one best answer)? (weight 25)
- Is the explanation accurate and teaching-focused? (weight 25)
Respond with JSON only: { "score": number 0-100, "feedback": string, "issues": string[] }
`;

export interface CriticGradeResult {
  score: number;
  feedback: string;
  issues: string[];
}
...
/**
 * Run Critic (reasoning-heavy model, e.g. Gemini 1.5 Pro) on a staging question.
 * Writes score (0-100) and feedback into aiGrade; does not change status.
 */
export async function runCriticGrading(
  prisma: any,
  env: any,
  stagingQuestionId: string
): Promise<CriticGradeResult> {
  const question = await prisma.stagingQuestion.findUnique({
    where: { id: stagingQuestionId },
  });
  if (!question) throw new Error('Staging question not found');
  ...
  if (env.GEMINI_API_KEY) {
    try {
      ...
      const prompt = `${CRITIC_RUBRIC}\n\nVignette: ${question.vignette}\nQuestion: ${question.question}\nCorrect: ${question.correctAnswer}\nOptions: ${JSON.stringify(question.options)}\nExplanation: ${(question.explanation || '').slice(0, 500)}`;
      const result = await model.generateContent(prompt);
      ...
      console.error('Critic grading error:', e);
      feedback = 'Critic run failed';
    }
  }
...
const CRITIC_AUTO_PROMOTE_THRESHOLD = 90;
const CRITIC_AUTO_REJECT_THRESHOLD = 70;

/**
 * Apply automation after critic: score > 90 → promote & delete; < 70 → delete; 70–90 → flag for human review.
 */
export async function applyCriticAutomation(
  prisma: any,
  stagingQuestionId: string,
  score: number
): Promise<'promoted' | 'rejected' | 'flagged_for_review'> {
  if (score > CRITIC_AUTO_PROMOTE_THRESHOLD) {
    await prisma.stagingQuestion.update({
      where: { id: stagingQuestionId },
      data: { status: 'graded' },
    });
    await promoteToLive(prisma, stagingQuestionId);
    await prisma.stagingQuestion.delete({ where: { id: stagingQuestionId } }).catch(() => {});
    return 'promoted';
  }
  if (score < CRITIC_AUTO_REJECT_THRESHOLD) {
    await prisma.stagingQuestion.delete({ where: { id: stagingQuestionId } });
    return 'rejected';
  }
  await prisma.stagingQuestion.update({
    where: { id: stagingQuestionId },
    data: { status: 'flagged_for_review', adminReview: `Critic score ${score}; human review required` },
  });
  return 'flagged_for_review';
}

/**
 * Run Critic (Pro) on pending questions then apply automation (promote / reject / flag).
 */
export async function processStagingQueueWithCritic(prisma: any, env: any, limit: number = 10) {
  ...
  for (const q of pending) {
    try {
      const { score } = await runCriticGrading(prisma, env, q.id);
      const status = await applyCriticAutomation(prisma, q.id, score);
      results.push({ id: q.id, status, score });
    } catch (error) {
      results.push({ id: q.id, status: 'error', error: (error as Error).message });
    }
  }
  return results;
}
