...
export const onRequestGet = authenticatedEndpoint(
  ...
  async (context) => {
    ...
    if (!res.ok) {
      ...
      return new Response(
        JSON.stringify({ error: 'Failed to get operation status', details: text.slice(0, 300) }),
    ...
    if (!data.done) {
      ...
      return new Response(
        JSON.stringify({
          data: {
            status: 'pending',
            message: 'Video generation in progress. Poll again in 10–15 seconds.',
    ...
    try {
      ...
      return new Response(
        JSON.stringify({
          data: {
            status: 'failed',
            error: 'Failed to retrieve or store video',
          },
        }),
        { status: 200, headers: { 'Content-Type': 'application/json', ...rateLimitHeaders } }
      );
