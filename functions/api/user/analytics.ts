...
async function calculateConditionMastery(prisma: ReturnType<typeof createEdgePrismaClient>, attempts: any[]): Promise<UserAnalytics['conditionMastery']> {
...
function calculateRecentActivity(attempts: any[]): UserAnalytics['recentActivity'] {
  ...
  return Array.from(activityMap.entries()).map(([date, stats]) => ({ date, questions: stats.questions, correct: stats.correct, accuracy: stats.questions > 0 ? (stats.correct / stats.questions) * 100 : 0 })).sort((a, b) => a.date.localeCompare(b.date));
}
...
async function getSRSStats(prisma: ReturnType<typeof createEdgePrismaClient>, userId: string): Promise<UserAnalytics['srsStats']> {
  ...
  const overdue = srsItems.filter((item) => { const dueDate = new Date(item.dueDate); dueDate.setDate(dueDate.getDate() + 1); return dueDate < now; }).length;
  const avgEasiness = srsItems.length > 0 ? srsItems.reduce((sum, item) => sum + item.easiness, 0) / srsItems.length : 2.5;
  const avgInterval = srsItems.length > 0 ? srsItems.reduce((sum, item) => sum + item.interval, 0) / srsItems.length : 1;
