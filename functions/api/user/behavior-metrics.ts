...
export const onRequestPost = authenticatedEndpoint(BehaviorMetricsPostSchema, async (context) => {
  ...
  try {
    ...
    const confidenceLevelInt =
      payload.confidenceLevel === undefined
        ? undefined
    ...
    const metrics = await prisma.userBehaviorMetrics.create({
      data: {
        ...
        scrollDepth:
          payload.scrollDepth === undefined ? undefined : Math.round(payload.scrollDepth),
        ...
        derivedRating:
          payload.derivedRating === undefined ? undefined : Math.round(payload.derivedRating),
