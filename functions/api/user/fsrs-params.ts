    ...
    if (personalizedParams) {
      ...
      const response: FSRSParamsResponse = {
        params: {
          ...
          systemModifiers: personalizedParams.systemModifiers as Record<string, {
            stabilityMultiplier: number;
            difficultyOffset: number;
            sampleSize: number;
            accuracy: number;
...
export async function onRequestPost(context: {
  ...
  try {
    ...
    console.log(`[FSRS-Params] Starting optimization for user ${auth.userId} with ${allSnapshots.length} snapshots`);
