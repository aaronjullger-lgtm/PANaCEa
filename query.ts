...
async function runQueries() {
  try {
    ...
    console.log('2. Questions by organ system (top 15):');
    const q2 = await prisma.$queryRaw`
      SELECT "system", COUNT(*) as count FROM "PreGeneratedQuestion" WHERE "system" IS NOT NULL GROUP BY "system" ORDER BY COUNT(*) DESC LIMIT 15
    `;
    console.log(q2);
    console.log();
