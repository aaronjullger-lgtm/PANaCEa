...
const ERROR_TAG_OPTIONS: { tag: ErrorTag; label: string; icon: React.ReactNode; color: string }[] = [
  { 
    ...
    color: 'text-purple-600 bg-purple-50 border-purple-200 hover:bg-purple-100 dark:text-purple-300 dark:bg-purple-900/30 dark:border-purple-700 dark:hover:bg-purple-900/50'
  },
  { 
    tag: 'misread_question', 
    label: 'Misread Question', 
    icon: <Eye className="w-4 h-4" />,
    color: 'text-blue-600 bg-blue-50 border-blue-200 hover:bg-blue-100 dark:text-blue-300 dark:bg-blue-900/30 dark:border-blue-700 dark:hover:bg-blue-900/50'
  },
  { 
    tag: 'guessing', 
    label: 'Guessing', 
    icon: <HelpCircle className="w-4 h-4" />,
    color: 'text-amber-600 bg-amber-50 border-amber-200 hover:bg-amber-100 dark:text-amber-300 dark:bg-amber-900/30 dark:border-amber-700 dark:hover:bg-amber-900/50'
  },
];
...
const ErrorTagger: React.FC<ErrorTaggerProps> = ({ onTagError, disabled = false }) => {
  ...
  if (selectedTag) {
    ...
    return (
      ...
      >
        <span>Tagged as</span>
        <span className={`inline-flex items-center gap-1 px-2 py-1 rounded-full border text-xs font-medium ${selected?.color}`}>
          {selected?.icon}
          {selected?.label}
        </span>
      </motion.div>
