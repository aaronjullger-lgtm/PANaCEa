...
function analyzeQuestion(text: string): DifficultyFactors {
  ...
  const labPattern = /\d+\.?\d*\s*(mg\/dL|g\/dL|mEq\/L|mmol\/L|U\/L|IU\/L|ng\/mL|μg\/L|%|mm\/hr|cells\/μL)/i;
  const hasLabValues = labPattern.test(text);
  
  // Check for multi-step reasoning indicators
  const multiStepIndicators = [
    'after', 'then', 'following', 'next step', 'subsequently',
    'initial', 'first-line', 'if', 'unless', 'despite',
  ];
  const hasMultipleSteps = multiStepIndicators.some(ind => 
    cleanText.toLowerCase().includes(ind)
  );
        ...
        <div className="flex items-center gap-1">
          {[1, 2, 3, 4].map((dot) => (
            <motion.div
              ...
              animate={{ 
                scale: dot <= (level === 'easy' ? 1 : level === 'medium' ? 2 : level === 'hard' ? 3 : 4) ? 1 : 0.8,
                opacity: dot <= (level === 'easy' ? 1 : level === 'medium' ? 2 : level === 'hard' ? 3 : 4) ? 1 : 0.3,
              }}
