...
export const SessionStatsOverlay: React.FC<SessionStatsOverlayProps> = ({
  ...
  return (
    ...
    >
      {/* Compact Header */}
      <div 
        className="bg-white/95 dark:bg-slate-800/95 backdrop-blur-sm rounded-xl shadow-lg border border-slate-200 dark:border-slate-700 overflow-hidden"
      >
        {/* Quick Stats Bar */}
        <button
        ...
        >
          <div className="flex items-center gap-4">
            ...
            {/* Accuracy */}
            <div className="flex items-center gap-1.5">
              <TrendingUp className={`w-4 h-4 ${sessionAccuracy >= 70 ? 'text-emerald-500' : sessionAccuracy >= 50 ? 'text-amber-500' : 'text-red-500'}`} />
              <span className={`font-semibold ${sessionAccuracy >= 70 ? 'text-emerald-600 dark:text-emerald-400' : sessionAccuracy >= 50 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400'}`}>
                {sessionAccuracy}%
              </span>
            </div>
            ...
            {summary && (
              <div className="flex items-center gap-1.5">
                <PieChart className={`w-4 h-4 ${summary.distributionScore >= 80 ? 'text-emerald-500' : summary.distributionScore >= 60 ? 'text-amber-500' : 'text-red-500'}`} />
              ...
              <div className="p-3 space-y-3">
                ...
                <div className="grid grid-cols-2 gap-3">
                  <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-2 text-center">
                    ...
                    <div className={`text-lg font-bold ${sessionAccuracy >= 70 ? 'text-emerald-600 dark:text-emerald-400' : sessionAccuracy >= 50 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400'}`}>
                      {sessionAccuracy}%
                    </div>
                  </div>
                  <div className="bg-slate-50 dark:bg-slate-700/50 rounded-lg p-2 text-center">
                    <div className="text-xs text-slate-500 dark:text-slate-400 mb-1">Last 10</div>
                    <div className={`text-lg font-bold ${recentAccuracy >= 70 ? 'text-emerald-600 dark:text-emerald-400' : recentAccuracy >= 50 ? 'text-amber-600 dark:text-amber-400' : 'text-red-600 dark:text-red-400'}`}>
                      {recentAccuracy}%
                    </div>
                  </div>
                </div>
                    ...
                    <div className="text-xs text-amber-600 dark:text-amber-500">
                      {needsCorrection.slice(0, 2).map(d => (
                        <span key={d.system} className="mr-2">
                          {d.system}: {d.driftPercent > 0 ? '+' : ''}{d.driftPercent}%
                        </span>
                      ))}
                    </div>
                  </div>
                        ...
                        <div className="flex-1 h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
                          <div 
                            className={`h-full rounded-full transition-all duration-300 ${
                              bar.isOver ? 'bg-amber-400' : 
                              bar.isUnder ? 'bg-blue-400' : 
                              'bg-emerald-400'
                            }`}
