...
export const MobileKeyboardInteractionAudit: React.FC<MobileKeyboardInteractionAuditProps> = ({
  ...
  const scanForKeyboardIssues = () => {
    ...
    inputs.forEach((input, index) => {
      ...
      if (tagName === 'input') {
        ...
        if (
          ...
          newIssues.push({
            ...
            severity: 'medium',
            description:
              'Email field should use type="email" to trigger appropriate mobile keyboard',
        ...
        if (
          ...
          newIssues.push({
            ...
            severity: 'medium',
            description:
              'Number field should use type="number" or inputmode="numeric" for numeric keyboard',
      ...
      if (!autocomplete && tagName === 'input') {
        ...
        const shouldHaveAutocomplete =
          ...
          inputElement.name?.match(
            /(email|username|name|password|tel|address|city|state|zip|country)/i
          );
    ...
    forms.forEach((form, index) => {
      ...
      const handlesEnterKey = form.querySelector(
    ...
    if (keyboardVisible && activeInput) {
      const activeElement =
        document.querySelector(`#${activeInput}`) ||
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-sm text-[var(--color-text-muted)] mt-1">
                Viewport: {window.innerWidth}×{viewportHeight}px •{' '}
                {keyboardVisible ? 'Keyboard visible' : 'No keyboard'} • Active:{' '}
                {activeInput || 'none'}
              </p>
            </div>
          </div>
                  ...
                  {testInputTypes.map((input) => (
                    ...
                    >
                      ...
                      <input
                        ...
                        autoComplete={
                          input.type === 'email' ? 'email' : input.type === 'tel' ? 'tel' : 'off'
                        }
                        inputMode={
                          input.type === 'number'
                            ? 'numeric'
                    ...
                    <div className="p-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)]">
                      ...
                      <ul className="space-y-2 text-sm text-[var(--color-text-secondary)]">
                        <li className="flex items-center gap-2">
                          ...
                          <span>
                            Use <code>type="email"</code> for email addresses
                          </span>
                        </li>
                        <li className="flex items-center gap-2">
                          <CheckCircle className="w-4 h-4 text-[var(--color-success)]" />
                          <span>
                            Use <code>type="tel"</code> for phone numbers
                          </span>
                        </li>
                        <li className="flex items-center gap-2">
                          <CheckCircle className="w-4 h-4 text-[var(--color-success)]" />
                          <span>
                            Use <code>type="number"</code> with <code>inputmode="numeric"</code>
                          </span>
                        </li>
                        <li className="flex items-center gap-2">
                          <CheckCircle className="w-4 h-4 text-[var(--color-success)]" />
                          <span>
                            Use <code>type="search"</code> for search fields
                          </span>
                        </li>
                      </ul>
                    </div>
                    ...
                    <div className="p-4 rounded-xl border border-[var(--color-border)] bg-[var(--color-bg-secondary)]">
                      ...
                      <ul className="space-y-2 text-sm text-[var(--color-text-secondary)]">
                        <li className="flex items-center gap-2">
                          <CheckCircle className="w-4 h-4 text-[var(--color-success)]" />
                          <span>
                            Always add <code>autocomplete</code> attributes
                          </span>
                        </li>
                        <li className="flex items-center gap-2">
                          <CheckCircle className="w-4 h-4 text-[var(--color-success)]" />
                          <span>
                            Use <code>autocomplete="email"</code> for email
                          </span>
                        </li>
                        <li className="flex items-center gap-2">
                          <CheckCircle className="w-4 h-4 text-[var(--color-success)]" />
                          <span>
                            Use <code>autocomplete="current-password"</code> for passwords
                          </span>
                        </li>
                      </ul>
                    </div>
            ...
            <div className="text-sm text-[var(--color-text-muted)]">
              {issues.length > 0 ? (
                <span>
                  ...
                  keyboard issues • High: {issues.filter((i) => i.severity === 'high').length} •
                  Medium: {issues.filter((i) => i.severity === 'medium').length} • Low:{' '}
