    ...
    return (
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <p className={`text-sm font-medium ${config.textColor}`}>{message.message}</p>
          {message.fieldId && (
            <p className="text-xs text-[var(--color-text-muted)] mt-1">
              Related to:{' '}
              <code className="font-mono bg-[var(--color-bg-secondary)] px-1 py-0.5 rounded">
                {message.fieldId}
              </code>
            </p>
          )}
  ...
  return (
    <div className={`space-y-3 ${className}`}>
      {showSummary && (hasErrors || hasWarnings) && (
        <div className="mb-2">
          <h3 className="text-sm font-medium text-[var(--color-text-primary)]">{summaryTitle}</h3>
...
export function useFormValidation(options: UseFormValidationOptions = {}) {
  ...
  const addMessage = React.useCallback(
    (message: Omit<ValidationMessage, 'id'>) => {
  ...
  const hasErrors = React.useMemo(() => messages.some((m) => m.severity === 'error'), [messages]);
