      ...
      <div className="bg-gradient-to-r from-[var(--color-bg-primary)]/40 to-[var(--color-bg-secondary)]/40 border border-[var(--color-border)] rounded-xl p-4">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className="text-sm text-slate-300 mt-1">Half in first 8 h from burn, half over next 16 h. Use LR.</div>
          </div>
        </div>
      </div>
        ...
        {total24 !== null && (
          <div className="space-y-2 p-4 bg-slate-800/50 rounded-xl border border-slate-700">
            ...
            <div className="text-slate-300 text-sm">
              First 8 h (from burn): <strong className="text-slate-100">{first8h?.toLocaleString()}</strong> mL
              &nbsp;|&nbsp; Next 16 h: <strong className="text-slate-100">{next16h?.toLocaleString()}</strong> mL
            </div>
          </div>
        )}
      </div>
