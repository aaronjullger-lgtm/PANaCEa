                  ...
                  >
                    {dateGrid.map((row, rowIdx) => {
                      ...
                      return (
                        <motion.button
                          key={`${rowIdx}-${colIdx}`}
                          initial={lowPower || prefersReducedMotion ? false : { opacity: 0, scale: 0.8 }}
                          animate={{ opacity: 1, scale: 1 }}
                          transition={
                            lowPower || prefersReducedMotion ? { duration: 0 } : { delay: colIdx * 0.01, duration: 0.2 }
                          }
                          whileHover={lowPower || prefersReducedMotion ? undefined : { scale: 1.2, zIndex: 10 }}
                          whileTap={lowPower || prefersReducedMotion ? undefined : { scale: 0.95 }}
