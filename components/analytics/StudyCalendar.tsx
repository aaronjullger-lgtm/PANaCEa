...
const VIEW_MODES: Record<CalendarViewMode, {
  label: string;
  ...
  mastery: {
    ...
    getIntensity: (data) => {
      ...
      const avgAccuracy = systems.reduce((sum, [, s]) => 
        sum + (s.total > 0 ? s.correct / s.total : 0), 0) / systems.length;
      return avgAccuracy;
    },
    getValue: (data) => {
      const systems = Array.from(data.systemBreakdown.entries());
      if (systems.length === 0) return 0;
      const avgAccuracy = systems.reduce((sum, [, s]) => 
        sum + (s.total > 0 ? (s.correct / s.total) * 100 : 0), 0) / systems.length;
    ...
    <div className="flex-shrink-0">
      ...
      <div className="grid grid-cols-7 gap-1 mb-1">
        {DAY_HEADERS.map((day, idx) => (
          <div key={idx} className="w-8 sm:w-10 text-center text-[10px] text-[var(--color-text-muted)]">
            {day}
          </div>
        ))}
      </div>
  ...
  const dataMap = useMemo(() => {
    ...
    map.forEach(data => {
      data.accuracy = data.questionsAnswered > 0 
        ...
        : 0;
      data.avgTimeMs = data.questionsAnswered > 0 
        ? Math.round(data.totalTimeMs / data.questionsAnswered) 
        : 0;
    });
