...
const ComparisonSection: React.FC<ComparisonSectionProps> = ({
  ...
  return (
    <div className="mb-6">
      <h3 className="text-sm font-semibold text-data-neutral uppercase tracking-wide mb-3">
        {title}
      </h3>
        ...
        <div className={`p-4 rounded-xl ${colorsA.bg} border ${colorsA.border}`}>
          ...
          <ul className="space-y-1.5">
            {itemsA.map((item, idx) => (
              <li
                key={idx}
                className="flex items-start gap-2 text-sm text-data-neutral"
              >
...
const SimilaritiesSection: React.FC<{ items: string[] }> = ({ items }) => (
  <div className="mb-6">
    ...
    <div className="p-4 rounded-xl bg-data-provisional/30 border border-data-provisional">
      <ul className="space-y-1.5">
        {items.map((item, idx) => (
          <li
            key={idx}
    ...
    <div>
      ...
      {triadItems.length > 0 && (
        <div className="mb-6">
          ...
          <div className="p-4 rounded-xl bg-[var(--color-data-fail)]/10 border border-[var(--color-data-fail)]/20">
            <ul className="space-y-1.5">
